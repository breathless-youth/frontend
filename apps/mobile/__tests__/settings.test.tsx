...
/**
 * S6 · 설정 — 내용물이 `RemoteScreen`(BY-333 2단계)으로 이관됐다. 카메라 권한 행 등 예전
 * 네이티브 UI 검증은 더 이상 이 화면의 몫이 아니다(웹이 소유). 여기서는 `/settings` 경로 +
 * 탭 공용 쿼리가 붙는지만 확인한다.
 */
...
describe("SettingsScreen", () => {
  it("/settings 경로 + 탭 공용 쿼리(userId·appVersion)로 조립한 URL을 로드한다", async () => {
    ...
    expect(screen.getByTestId("settings-webview").props.source).toEqual({
      uri: "https://web.test/settings?userId=7&appVersion=1.4.2&share=1&cameraGate=1&nativeUpdateGate=1&guestAuth=1",
    });
  });
});

 ...
 * - 앱 스킴: `focusmakers://social/join?code=1234` (`app.json`의 `scheme`)
 * - 운영 웹 호스트의 https 링크: `https://web.focusmakers.app/social/join?code=1234` (딥링크 등록 도메인)
 ...
 *
 * `link` 키 이름은 BE 알림 API가 정해지면 그에 맞춘다(현재 계약 없음).
 */
export const PUSH_LINK_KEY = "link";
export const PUSH_HOME_ROUTE = "/";

/** `app.json`의 `scheme`. 둘 다 앱을 연다. */
const APP_SCHEMES = new Set(["focusmakers", "focuson"]);
/** `app.json` intentFilters·associatedDomains에 등록된 웹 호스트. */
const DEEP_LINK_HOSTS = new Set(["web.focusmakers.app", "web.sunqstudio.kr"]);

const URL_PATTERN = /^([a-z][a-z0-9+.-]*):\/\/([^/?#]*)([^?#]*)(\?[^#]*)?/i;

/** 주소 하나를 경로로. 허용 범위 밖이면 null. */
export function routeFromPushLink(link: string | null | undefined): string | null {
  if (!link) return null;
...
export function resolvePushRoute(data: Record<string, string>): string {
  return routeFromPushLink(data[PUSH_LINK_KEY]) ?? PUSH_HOME_ROUTE;
}

...
it("속성이 없는 이벤트는 properties 키 자체를 싣지 않는다", () => {
  ...
  expect(received).toEqual([{ name: "permission_denied_settings_opened", atMs: 1_000 }]);
});
...
it("sink가 없으면 보관했다가 붙는 순간 순서대로 넘긴다 — 권한 거부 화면이 탭을 덮은 동안의 이벤트", () => {
  ...
  expect(received.map((event) => [event.name, event.atMs])).toEqual([
    ["permission_denied_settings_opened", 1_000],
...
it("나중에 붙은 sink가 활성이고, 그것이 떨어지면 먼저 붙은 sink로 돌아간다", () => {
  ...
  expect(second.received.map((event) => event.name)).toEqual(["permission_denied_settings_opened"]);
  expect(first.received.map((event) => event.name)).toEqual(["permission_denied_left"]);
});
...
it("모든 sink가 떨어지면 다시 보관한다", () => {
  ...
  expect(next.received.map((event) => event.name)).toEqual(["permission_denied_settings_opened"]);
});

it("보관 상한을 넘으면 오래된 것부터 버린다 — 웹뷰가 오래 안 뜨는 동안의 메모리 상한", () => {
  for (let i = 0; i < 105; i += 1) {
    trackNativeEvent("webview_load_failed", { path: `/${i}`, reason: "error" });
  }

  const { received } = collector();

  expect(received).toHaveLength(100);
  expect(received[0]?.properties).toEqual({ path: "/5", reason: "error" });
  expect(received.at(-1)?.properties).toEqual({ path: "/104", reason: "error" });
});

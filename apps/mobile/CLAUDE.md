...
- 백그라운드 복귀는 `lib/appStateAnalytics.ts`가 `app/_layout.tsx`의 AppState 감시에서 판정해 `app_foregrounded {background_sec}`로 넘긴다 — iOS `inactive`와 앱 시작 직후 첫 active는 세지 않고, 떠난 시점은 이벤트로 남기지 않는다(Amplitude 세션 종료·`study_session_paused {BACKGROUND}`가 표시). 웹 `app-state` 릴레이(#97)는 발신자가 없었고 웹뷰 수만큼 중복되는 구조라 이쪽으로 옮겼다. S2-3 이탈은 `beforeRemove`에서 한 번만(버튼·자동 복귀·하드웨어 백·스와이프 백 전부, 사유 없으면 `back`). **클릭 자체나 다른 이벤트와 1:1인 것은 이벤트로 두지 않는다**(2026-09-05 재검토 — 노출·재시도 터치·하드웨어 백 등을 뺐다. 설계 문서 "재검토" 절).

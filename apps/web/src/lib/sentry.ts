 ...
 * 실패를 삼키는데(각 catch의 주석 참고), 그중 핵심 기능이 죽는 실패(비전 측정 로드·카메라
 * 획득·세션 제출)는 삼켜지는 순간 아무 데도 보이지 않게 된다 — 리텐션이 낮을 때 "측정
 ...
 * - 핵심 기능이 죽는 실패에만 쓴다. 다음 실행에서 복구되는 무해한 실패(온보딩 열람 기록,
 *   업데이트 공지)에 쓰면 잡음이 신호를 덮는다.
 */

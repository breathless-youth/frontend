 ...
 *
 * ## 왜 index.css 시맨틱 변수(`bg-background`·`text-foreground` 등 라이트/다크 자동 전환 유틸)를 쓰지 않는가
 *
 * `src/index.css`의 시맨틱 변수는 `prefers-color-scheme`에 따라 라이트/다크가 갈린다. 그런데
 * 이 플로우는 시스템 테마와 무관하게 **항상 다크 오버레이 위**에 그려진다(Figma 5프레임 모두 다크 고정,
 * 스펙 "이 화면은 라이트/다크 테마 분기가 없다"). 시맨틱 변수를 쓰면 라이트 모드 기기에서
 * 라이트값이 나와 다크 배경 위 대비가 무너진다 — 그래서 토큰의 **다크값을 직접** 집어 쓴다
 * (`features/study-session/sessionTheme.ts`가 세션 오버레이에 쓴 것과 같은 이유).
 *
 * ## 왜 일부 값은 시맨틱 토큰이 아닌가
 *
 ...
 *
 * (`apps/mobile/components/onboarding/coachOverlayTheme.ts`에서 이식 — BY-334 온보딩 웹 이관.
 * RN 그림자 오브젝트(`shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius`/`elevation`)만
 * CSS `boxShadow` 문자열로 바뀌었고 나머지 값은 그대로다.)
 */
...
export const coachOverlay = {
  ...
  cardShadow: { boxShadow: "0 16px 40px rgba(0,0,0,0.45)" },
...
export const coachTokenColors = {
   ...
   *
   * 그래서 이 상수를 실제 세션 화면(`features/study-session`)의 근거로 가져가지 말 것 — 값이
   * 같아질 가능성이 높다는 것과 별개로, 확정 시 고쳐야 할 지점이 두 화면으로 흩어지면 한쪽만
   * 고치고 닫힐 위험이 있다. 세션 화면은 자체 프레임과 자체 스펙(`SCR-S3-*`)을 근거로 삼는다.

 ...
 *
 * 헤어라인 색은 Figma 실측값(#EFF1F3)이 아니라 `border` 토큰이다 — 원본은 이 값을
 * 변수에 바인딩하지 않아 다크 모드 값이 없고, 그대로 쓰면 카드 배경 위에 극단적으로
...
export function SettingsSection({ label, caption, className, children }: SettingsSectionProps) {
  ...
  return (
    <div className={className}>
      ...
      {caption !== undefined && (
        <p className="text-text-tertiary mt-2.5 px-1 text-xs leading-[14px]">{caption}</p>
      )}
    </div>
  );
}

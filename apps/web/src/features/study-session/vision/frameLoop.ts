 ...
 * 쌓는다. 추론이 200ms보다 오래 걸리는 순간(저사양 기기·CPU 폴백에서 실제로 일어난다) 호출이
 * 겹쳐 쌓이고, MediaPipe `detectForVideo`는 재진입 안전이 보장되지 않는다. 게다가 밀린 호출을
 * 몰아 처리하면 프레임 하나당 200ms라는 전제가 깨져 배터리만 태운다.
 *
 * 그래서 **직전 추론이 안 끝났으면 이번 프레임을 통째로 버린다.** 감지 판정의 최소 유지시간이
 * 0.5초라(설계 §3) 프레임 한두 개를 건너뛰어도 판정은 바뀌지 않는다.
 *

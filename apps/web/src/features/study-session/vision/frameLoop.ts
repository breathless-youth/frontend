 ...
 *
 * 그래서 **직전 추론이 안 끝났으면 이번 프레임을 통째로 버린다.** 밀린 만큼 몰아서 처리하지
 * 않는다.
 *
 * ⚠️ 지금은 추론(int8 213~371ms)이 주기(200ms)보다 길어 **버리는 것이 예외가 아니라 정상
 * 상태**다 — 실효 샘플 간격을 정하는 건 `FRAME_INTERVAL_MS`가 아니라 추론 시간이고, 그만큼
 * CPU는 쉬지 않는다(BY-305 실측, `visionConfig.ts` 주석). 실효 간격이 PHONE의 확정
 * 유지시간(500ms)보다는 짧아 판정 자체는 성립한다.
 *

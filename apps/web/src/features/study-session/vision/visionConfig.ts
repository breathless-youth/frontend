 ...
 *
 * 판정 영향: 유지시간 디바운스는 벽시계 기반(`../detection.ts`)이라 주기를 늘려도 판정
 * 로직 자체는 유효하다. 영향은 **이 주기가 만드는 신호에만** 온다 — AWAY·PHONE만 여기서
 * 나오고 DEVICE는 가속도 센서(20Hz) 경로라 무관하다. AWAY는 `enterMs` 1500ms라 최소 한
 * 샘플이 보장되므로, 실제 취약한 것은 **`enterMs` 500ms인 PHONE 하나다**(주기보다 짧다).
 *
 * PHONE에서 세 가지가 생긴다. (1) 확정이 늦어진다 — 관측 지연(0~1초)이 앞에 붙어 실측
 * ~0.5~1.5초. (2) **두 샘플 사이에서 시작해 끝난 신호는 아예 관측되지 않는다** — 지연이
 * 아니라 누락이고, 비집중이 집중으로 계산되어 순공시간이 과대 집계된다. (3) 반대로 한
 * 샘플에만 걸린 짧은 신호는 다음 샘플까지 참으로 유지되어 실제보다 길게 집계된다.
 * 셋은 서로 다른 문제이므로 BY-304에서 각각 측정한다.
 */

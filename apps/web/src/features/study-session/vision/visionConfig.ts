 ...
 *
 * 값의 근거: 인체공학 문헌에서 책상 위 책·폰을 볼 때 목 굴곡이 30~50°, 모니터를 볼 때 0~15°다.
 * 25°는 그 사이다. 실측 덩어리의 구간별 `headPitch` 분포(눈 감기 vs 내려다봄·휴대폰)로 확정한다.
 *
 * ⚠️ **부호는 첫 실측에서 확인한다.** MediaPipe 자세 행렬의 메모리 배치(행/열 우선)가 문서에 없어
 * 아래를 볼 때 양수가 되도록 가정하고 계산했다(`faceLandmarker.ts`의 `headPitchDegOf`). 패널의
 * `고개` 값이 아래를 볼 때 음수로 가면 그 함수의 부호 하나만 뒤집는다.
 */
export const HEAD_PITCH_DOWN_DEG = 25;

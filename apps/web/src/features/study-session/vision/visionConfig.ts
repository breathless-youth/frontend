 ...
 * 여유가 얇으므로 피험자를 늘려 다시 정한다. 지금은 개인별 보정의 하한으로만 쓰인다.
 */
export const SLEEP_THRESHOLDS = { eyeClosure: 0.45 } as const;
 ...
 * 1이 감김이라 비율이 아니라 간격으로 옮긴다. 스파이크에서 뜬 눈이 0.18~0.24이고 고정 임계가
 * 0.45였으니, 그 피험자에게 같은 임계가 나오는 간격이다.
 */
export const EYE_CALIBRATION_DELTA = 0.25;

/**
 * 보정된 임계의 하한. **잠정값이다.** 임계는 뜬 눈 기준값 + `EYE_CALIBRATION_DELTA`이고 이 값
 * 아래로는 내려가지 않는다. 상한은 없다.
 *
 * 하한이 고정 임계와 같다. **보정은 임계를 올리기만 하고 내리지는 못한다.** 내릴 수 있게 두면
 * 보정이 판정을 느슨하게 만드는 방향이 열리는데, 그쪽은 깨어 있는 사람을 졸음으로 잡는 오탐이다.
 * 눈이 큰 사람은 감아도 점수가 낮아 고정 임계에 못 미칠 수 있고 그 사람에게는 하한을 내리는 것이
 * 맞지만, 그 데이터가 실기기에서 나오기 전까지는 오탐을 막는 쪽으로 둔다.
 *
 * 상한을 두지 않는 이유도 방향이다. 뜬 눈이 0.5로 읽히는 사람은 감으면 0.8~0.9까지 갈 수 있는데,
 * 상한 0.65로 자르면 뜬 눈과 0.15밖에 차이가 안 나 흔들리는 순간 깨어 있는데 졸음으로 찍힌다.
 * 상한이 없으면 감은 눈이 임계에 못 미치는 사람을 놓칠 수는 있어도, 깨어 있는 사람을 졸음으로
 * 잡는 쪽으로는 틀리지 않는다. 그 사람의 감은 눈 값이 얼마인지는 실기기 데이터로 확인한다.
 */
export const EYE_THRESHOLD_MIN = SLEEP_THRESHOLDS.eyeClosure;

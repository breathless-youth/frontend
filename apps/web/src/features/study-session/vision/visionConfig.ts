 ...
 * 않는다. 거꾸로 int8이 있어야 `FRAME_INTERVAL_MS`(500ms)에서 유휴가 생긴다 — fp32(401~594ms)
 * 였다면 500ms에서도 여전히 포화다. 둘은 대체재가 아니라 순서가 있는 조합이다.
 *
...
/**
 * 프레임 처리 주기 — 500ms(2 FPS). (BY-305, 2026-08-19에 200ms에서 확대)
 *
 * ## 왜 200ms를 떠났나
 *
 ...
 *
 * ## 왜 1000ms가 아니라 500ms인가
 *
 * 1000ms도 시도했다(듀티 ~30%). 하지만 **PHONE의 확정 유지시간(`enterMs` 500ms)보다 샘플링
 * 간격이 길어지면**, 1초 미만의 폰 사용이 두 샘플 사이에 통째로 들어가 어느 프레임에도 걸리지
 * 않는다 — 지연이 아니라 **누락**이고, 비집중이 집중으로 계산되어 순공시간이 과대 집계된다
 * (`mvp-scope.md`의 "측정할 수 없는 시간은 집중으로 인정하지 않는다"와 반대 방향).
 *
 * 500ms는 그 경계를 넘지 않는 최대값이다. 관측 지연이 최대 1주기(500ms)라 `enterMs`와 같고,
 * 신호가 두 샘플 사이로 사라지는 구간이 생기지 않는다. 동시에 **기존의 실효 샘플 간격
 * (0.4~0.6초)과 사실상 같은 값**이라 감지 특성 변화가 가장 작다 — 명목값을 이미 일어나고
 * 있던 실제 동작에 맞춘 셈이다.
 *
 ...
 * **관측 지연(0~500ms) + `enterMs`(500ms) = 0.5~1.0초**(평균 ~0.75초)다. AWAY는 `enterMs`가
 * 1500ms라 영향이 없고, DEVICE는 가속도 센서 경로(20Hz)라 이 주기와 무관하다.
 *
 * 반대 방향 오차도 있다 — 한 샘플에만 걸린 짧은 신호는 다음 샘플까지 참으로 유지되어 실제보다
 * 길게 집계된다. 지연·누락·과대 집계는 서로 다른 문제이므로 BY-304에서 각각 측정한다.
 *
 * ⚠️ **저사양 기기는 미검증이다.** 추론이 500ms를 넘으면 다시 포화되어 이 값의 효과가 사라진다
 * (A13급에서 실측 필요 — `WebKit.WebContent` 프로세스 CPU%로 확인한다).
 */

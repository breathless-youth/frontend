 ...
 * 공부하는 동안 지나가며 재므로 더 길게 본다. 첫 창이 찰 때까지는 고정 임계를 쓰므로 보정이
 * 늦어도 판정이 멈추지는 않는다.
 *
 ...
 * 맞지만, 그 데이터가 실기기에서 나오기 전까지는 오탐을 막는 쪽으로 둔다. 실기기 A17이 그
 * 방향을 잰다.
 *
 * 상한은 블렌드셰이프 감김의 관례적 문턱 0.5 언저리에서 멀어지지 않게 하는 선이다.
 */
export const EYE_THRESHOLD_MIN = SLEEP_THRESHOLDS.eyeClosure;
export const EYE_THRESHOLD_MAX = 0.65;

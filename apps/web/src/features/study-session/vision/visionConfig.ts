 ...
 * 로직 자체는 유효하다. 하지만 **확정 유지시간보다 샘플링 간격이 길어졌다** — PHONE·DEVICE의
 * `enterMs`가 각 500ms라 1초 주기에서는 한 샘플도 못 얻는 구간이 생긴다. 그래서 영향이 두
 * 가지다: (1) 확정이 최대 +1주기 늦어지고(폰 사용 ~0.7초 → ~1.5초), (2) **두 샘플 사이에서
 * 시작해 끝난 신호는 아예 관측되지 않는다.** 지연과 누락은 다른 문제이므로 BY-304에서 둘 다
 * 측정한다 — 누락률을 재지 않으면 순공시간이 조용히 과대 집계된다.
 */

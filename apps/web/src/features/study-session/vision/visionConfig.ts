 ...
 *
 * ⚠️ **int8만으로는 발열이 잡히지 않는다.** 주기(200ms)보다 추론이 여전히 길어 백투백
 * 포화가 유지되므로 CPU 듀티는 ~100% 그대로다(`FRAME_INTERVAL_MS` 주석). 프레임당 비용을
 * 낮춰 둔 것일 뿐, 유휴를 만들려면 주기 조정이 따로 필요하다.
 *
...
/**
 * 프레임 처리 주기 — 200ms(명목 5 FPS).
 *
 * 가장 짧은 판정이 0.5초(폰 사용 확정 유지시간)라 그 안에 2~3프레임이 들어간다. 초당 30프레임을
 * 돌려도 판정 정확도는 그대로이고 배터리와 발열만 늘어난다(설계 §3).
 *
 * ⚠️ **이 값은 명목일 뿐 실효 주기가 아니다 — BY-305 실측(2026-08-14).** 추론이 주기보다 오래
 * 걸려서(int8 213~371ms, fp32 401~594ms) `frameLoop`의 busy-guard가 프레임을 버리고, 추론은
 * 끝나는 즉시 다음이 시작되는 **백투백 루프로 돈다 — CPU 듀티 ~100%.** 실효 샘플 간격은 이
 * 상수가 아니라 추론 시간이 정한다.
 *
 ...
 *
 * **1000ms로 늘렸다가 되돌렸다.** 듀티는 ~30%로 떨어지지만 간격이 PHONE의 `enterMs`(500ms)를
 * 넘어서, 1초 미만의 폰 사용이 두 샘플 사이에 떨어지면 지연되는 게 아니라 **아예 관측되지
 * 않는다**(비집중이 집중으로 계산되어 순공시간 과대 집계). 감지 정책을 함께 정하지 않고
 * 주기만 늘릴 수는 없다 — BY-304에서 지연·누락·과대 집계를 측정한 뒤 주기와
 * `DEFAULT_DETECTION_PARAMS`를 같이 결정한다.
 */

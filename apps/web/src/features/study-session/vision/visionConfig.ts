...
/**
 * 쉬는 자세로 인정하려면 뜬 눈으로 고개를 멈춘 채 이만큼의 얼굴 틱(2초 × 5 = 10초)이 이어져야 한다 —
 * `glanceRule.ts`. **잠정값이다.**
 *
 * 짧으면 책을 읽는 동안 흔들리는 점수가 우연히 몇 틱 연속 뜸으로 읽혀 읽는 자세가 쉬는 자세로 잡히고,
 * 그 뒤의 감김이 통과한다(2026-09-22 실측에서 뜸 하나로 풀던 첫 구현이 정확히 그렇게 졸음을 세웠다).
 * 길면 앉은 사람이 감기 전에 그만큼 눈을 뜨고 있어야 하는데, 10초는 세션 시작 직후 잠드는 경우
 * 말고는 항상 성립한다.
 */
export const GLANCE_REST_OPEN_TICKS = 5;

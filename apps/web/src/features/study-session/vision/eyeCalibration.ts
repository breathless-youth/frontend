 ...
 * 하나 앞이다. 보간하지 않는 것은 실제로 관측된 값이어야 임계와 직접 비교할 수 있기 때문이다.
 * 인덱스를 하나 밀어 쓰면 경계에서 답이 뒤집힌다 —
...
export function calibrateEye(
  ...
  // 하한도 상한도 없다. 근거는 `visionConfig.ts`의 `EYE_CALIBRATION_DELTA` 아래 주석.
  const threshold = baseline + EYE_CALIBRATION_DELTA;

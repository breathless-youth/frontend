...
/**
 * 체크 아이콘 원형 (Figma `check-circle` 63:587 + `icon/check` 63:588).
 *
 * SVG path는 Figma export(`icon/check` 63:588)에서 **그대로 옮긴 값**이다 — 손으로 그리지
 * 않는다. 유일한 변경은 `stroke`를 `#1B64DA`(= `brand/primary`의 라이트 값)에서
 * `currentColor`로 바꾼 것이다: 이 화면은 테마 반응형이라 다크에서 아이콘도 `brand/primary`의
 ...
 *
 * 아이콘은 **장식**이다 — "저장됐다"는 의미는 타이틀 텍스트가 전달한다(아이콘 단독 전달 금지).
 */

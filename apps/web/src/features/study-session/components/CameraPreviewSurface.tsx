    ...
    >
      {isRunning ? (
        <video
          ...
          //
          // ## 74%라는 어긋남 자체를 줄이는 쪽 (BY-336, 2026-07-31)
          //
          // 위 표의 74%는 **스트림 비율을 주어진 값으로 놓고** 계산한 것이다. 그 전제를 흔드는
          // 시도가 회전 시 카메라 재오픈이다 — `getUserMedia`에 지금 화면 비율을
          // `aspectRatio: { ideal }`로 함께 요청한다(`cameraConstraints`). 회전하면 잘리는 축이
          // 좌우↔상하로 뒤집혀 가로에서 얼굴이 띠처럼 잘린다는 실기기 관측이 계기였다.
          //
          // ⚠️ `ideal`은 소프트 제약이라 **기기가 무시할 수 있다.** 그래서 이 표시 경로는
          // 어떤 비율이 와도 깨지지 않아야 한다 — 아래 `object-fit` 분기가 그 방어선이고,
          // 실제로 무엇이 왔는지는 `camera:stream` 진단이 열 때마다 남긴다.
          //

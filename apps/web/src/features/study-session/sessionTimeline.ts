 ...
 *
 * **확정(2026-07-26 리더 확정): 현재 동작(버림) 유지.** 1초 미만 구간이 이벤트에서 사라져도
 * `studySec`/`focusSec` 집계에는 이미 반영돼 있어 값이 서로 안 맞아 보일 수 있다는 것을 알고
 * 내린 결정이다(선택지 ⓐ). `computeSessionTotals`와 임계값을 일치시키지 않는다 — 이 값을
 * 다시 바꾸려면 새 리더 결정이 필요하다.
 */

 ...
 * 과목별 시간 파생 — 순수 함수.
 *
 * **별도 타이머가 없다.** 세션 타이머(`studySec`/`focusSec`)가 유일한 원천이고, 과목을 고른 시점의
 * 값을 기억해 두었다가 다음 전환·스냅샷·제출 때 그 차이를 그 과목에 얹는다. 그래서 일시정지·비집중에서
 * 세션 타이머가 멈추면 과목 시간도 저절로 멈춘다.
 *
 * 측정 단위는 **과목**이다. 할 일은 체크리스트일 뿐 시간이 붙지 않는다.
 * 선택 없이 흐른 시간은 어느 과목에도 쌓이지 않는다(과목 없는 시간 — 스펙 §4).
 */

/** 지금 시간이 쌓이는 과목의 id. 선택이 없으면 `null`이다. */
...
export interface SubjectTimeTracker {
  /** 이미 확정된 구간의 합. 같은 과목을 여러 번 골랐어도 과목당 한 건이다. */
  readonly entries: readonly SubjectTimePayload[];
  /** 지금 고른 과목과, 골랐을 때의 세션 타이머 값. */
  readonly current: {
...
function addEntry(
  ...
  const index = entries.findIndex((entry) => entry.subjectId === next.subjectId);
  if (index === -1) {
    return [...entries, next];
  }
  const merged = {
...
export function createSubjectTimeTracker(
  ...
  return {
    ...
    current: last === undefined ? null : { subjectId: last.subjectId, since: base },
  };
}
...
function settle(tracker: SubjectTimeTracker, totals: SessionTotalsLike): SubjectTimePayload[] {
  ...
  const { subjectId, since } = tracker.current;
...
/**
 * 과목을 고른다(`null`이면 선택 해제). 같은 과목을 다시 고르면 아무 일도 없다 — 구간을 끊어
...
export function selectSubjectTime(
  ...
  return {
    ...
    current: next === null ? null : { subjectId: next, since: totals },
  };
}

/**
 * 서버로 보낼 과목 시간 — 확정 구간 + 지금 선택 구간의 `totals` 시점 차이. 둘 다 0인 과목은
 * 빼되, **지금 선택한 과목은 0이어도 남긴다** — 복원 시 마지막 항목으로 선택을 되살리기 때문이다.
 */
export function materializeSubjectTimes(
  tracker: SubjectTimeTracker,
  totals: SessionTotalsLike,
): SubjectTimePayload[] {
  const currentId = tracker.current?.subjectId ?? null;
  return settle(tracker, totals).filter(
    (entry) => entry.studySec > 0 || entry.focusSec > 0 || entry.subjectId === currentId,
  );
}

/** 화면 표시용 — 이 세션에서 해당 과목에 쌓인 시간. */

 ...
 * (`vision/visionConfig.ts`의 `FRAME_INTERVAL_MS`) 함께 올렸다 — **주기와 짝인 값이라 따로 바꾸면
 * 안 된다.** 2026-09-22 저녁 한 회차만 500ms·1500/500으로 돌렸다가 함께 되돌렸다.
 *
 ...
 *
 * 실제 확정까지 걸리는 시간은 관측 지연(0~1주기)이 더해진다 — `vision/visionConfig.ts` 주석 참고.
 */

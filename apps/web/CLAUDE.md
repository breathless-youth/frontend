...
- **`environment`는 `import.meta.env.MODE`를 쓰지 않는다.** `vite build`면 Preview든 Production이든 `"production"`이라 두 배포가 한 통에 섞인다. `vite.config.ts`의 `define`이 Vercel 시스템 변수를 주입한다 — `__DEPLOY_ENV__`(`VERCEL_ENV`) · `__RELEASE__`(커밋 SHA 7자리) · `__WEB_VERSION__`(apps/web/package.json의 version, YY.WW.P). Vercel env를 새로 설정할 필요가 없는 값들이다.
...
- **네이티브 셸 이벤트(`source: "native"`, 2026-09-04)** — 하단 탭 터치·카메라 권한 게이트 결과·권한 거부 안내(S2-3) 행동·권장 업데이트 알림창 응답·알림 탭·초대 딥링크·웹뷰 로드 실패/재시도는 웹이 알 수 없어 네이티브가 브리지 `track-event`로 넘기고, `useNativeAnalyticsRelay`(`lib/nativeAnalytics.ts`)가 `trackNativeShellEvent`로 전송한다. **카탈로그(이름·속성)는 `apps/mobile/lib/nativeAnalytics.ts`가 소유한다** — 웹은 이름을 해석하지 않고 `parseToWebMessage`가 형식(snake_case·원시값)만 검증한다. `time`은 네이티브가 기록한 발생 시각이다(포커스된 웹뷰가 준비될 때까지 네이티브 큐에 머물다 늦게 올 수 있다 — 세션 귀속은 SDK가 전송 시각으로 판단하므로 영향 없음). 훅은 **구독을 건 뒤** `analytics-ready`를 보낸다 — 순서를 바꾸지 말 것(네이티브가 신호를 받자마자 큐를 비우므로, 그 사이 도착한 이벤트가 구독자 없이 버려진다. `nativeAnalytics.test.tsx`가 고정).
- **세션 내부·홈·설정·복구 이벤트(BY-616 확장, 2026-09-05)** — 룸 안의 행동은 웹이 알므로 웹이 직접 찍는다. 함수는 `lib/amplitude.ts` 끝의 "BY-616 확장" 절, 목록·속성은 `docs/superpowers/specs/2026-09-04-native-analytics-bridge-design.md`. 규칙 셋: ① `room_type`은 BY-472의 `useStudyRoomSession` 값을 그대로 쓴다(개인/그룹 분리). ② 상태 전이 이벤트(`study_session_paused/resumed/distracted`)는 **전이가 실제로 일어났을 때만** 찍는다 — `applyState`가 같은 상태를 무시하면 타임라인 참조가 그대로라 아무것도 나가지 않고, `pause`/`resume`은 전이 전후 상태를 비교한다(`useStudyRoomSession.analytics.test.tsx`가 고정). ③ 권한 **상태**는 이벤트가 아니라 user property(`camera_permission_granted`)다 — 게이트의 분기 결과는 네이티브 `camera_permission_gate_resolved`가 갖는다.
- **2차 확장·최종 검토(2026-09-05)** — 온보딩 가이드 진입(`guide_entered {entry}`)·스텝·종료, 기록 달력 날짜·월 이동(스와이프 포함), 설정 행, 프로필 저장 제출·결과, 결과 화면 확인·비집중 펼치기, 세션 안내 확인, 세션 회전(`session_orientation_changed`)·심플 모드 토글, 방 생성 실패, 에러 재시도·폴백 새로고침, 뒤로가기 헤더, 웹 강제 업데이트 모달 노출·스토어 이동까지 명시 이벤트가 있다(전체 목록은 설계 문서 "최종 카탈로그"). **원칙: 사용자 사건은 전부 명시 이벤트로 남기고, autocapture `[Amplitude] Element Clicked`는 SDK 소유의 안전망으로만 본다** — 요소 텍스트만 알고 의미(스텝·룸 종류·결과)는 모르며 스와이프·회전·서버 응답은 아예 못 본다. 같은 순간에 다른 질문의 이벤트가 겹치는 것은 중복이 아니다(종료 요청 ↔ 종료 확정, 비집중 건별 ↔ 종료 집계). 중복은 **같은 사건을 같은 뜻으로 두 번** 찍는 것뿐이다 — `study_session_started`는 복원 진입에도 나므로 `restored`로 가른다(완주율 분모는 `restored = false`). 규칙: `ErrorState`는 `screen` prop이 필수다(계측 식별자). 가이드 스텝은 `stepMethodRef`에 수단을 적어 두고 effect에서 찍는다 — 핸들러마다 찍으면 실제 전이 없이도 이벤트가 난다.

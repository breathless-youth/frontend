...
- **세션 내부·홈·복구 이벤트(BY-616 확장, 2026-09-05)** — `study_session_paused/resumed`(상태 전이의 단일 통로 `applyState`에서 실제 전이가 일어났을 때만, `room_type` 포함), `focus_start_tapped {destination}`, `session_recovery_prompted`. 권한 **상태**는 이벤트가 아니라 user property(`camera_permission_granted`)다 — 게이트의 분기 결과는 네이티브 `camera_permission_gate_resolved`가 갖는다.
- **2차 확장·재검토(2026-09-05)** — 온보딩 가이드 스텝·종료(`guide_step_viewed/finished`)와 프로필 저장 결과(`profile_save_succeeded/failed`)만 명시 이벤트다. **버튼 클릭 자체는 이벤트로 만들지 않는다**: autocapture가 모든 클릭을 `[Amplitude] Element Clicked`로 요소 텍스트·aria-label·Page Path와 함께 잡으므로 콘솔의 Labeled Event로 대체한다(종료 버튼·설정 행·결과 닫기·오류 재시도 등을 이 기준으로 뺐다). 비집중 건별 이벤트도 볼륨 때문에 빼고 `study_session_ended`의 `away_count/phone_count/device_count/pause_count`로 접었다. 가이드 스텝은 `stepMethodRef`에 수단을 적어 두고 effect에서 찍는다.

...
- **2차 확장(2026-09-05)** — 온보딩 가이드 스텝·종료, 기록 달력 날짜·월 이동, 설정 행, 프로필 저장, 결과 화면 확인·비집중 펼치기, 세션 안내 확인, 에러 재시도·폴백 새로고침, 뒤로가기 헤더, 웹 강제 업데이트까지 명시 이벤트가 있다(목록은 설계 문서 "2차 확장" 표). 규칙: `ErrorState`는 `screen` prop이 필수다(계측 식별자). 가이드 스텝은 `stepMethodRef`에 수단을 적어 두고 effect에서 찍는다 — 핸들러마다 찍으면 실제 전이 없이도 이벤트가 난다. autocapture 클릭과 겹치는 건 의도다: autocapture는 요소 텍스트만 알고 의미(스텝·오늘 여부·룸 종류)는 모른다.

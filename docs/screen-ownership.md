...
`apps/web`은 현재 `HomePage`(소개)·`RoomPage`(S3-1~S3-4 확정 디자인 적용, 가로·종료 플로우는 WG3/WG4)를 제공한다. 스터디룸 화면은 위 표 기준으로 이 앱에 구현되어 모바일 WebView가 로드한다. 독립 브라우저 서비스로도 그대로 접근 가능하다.

...
| ID    | 화면                                | 상태 | 비고                                                  |
| ----- | ----------------------------------- | :--: | ----------------------------------------------------- |
| S1    | 홈                                  | [ ]  | `app/(tabs)/index.tsx` (현재 데모 진입 버튼만)        |
| S2-2  | 카메라 권한 요청(OS 다이얼로그)     | [ ]  | 커스텀 UI 없음 — `app.json`의 권한 문구·트리거 순서만 |
| S2-3  | 카메라 권한 거부 안내               | [ ]  |                                                       |
| S5    | 공부 기록(달력·통계·리스트)         | [ ]  | 신규 탭                                               |
| S6    | 설정                                | [ ]  | 신규 탭                                               |
| G1~G5 | 온보딩 가이드(5단계, 하나의 플로우) | [ ]  | 최초 '집중 시작' 탭 시 자동 실행                      |
...
| S3-1 | 세션 프리뷰                                                   | [ ]  |      |
| S3-2 | 세션 비집중                                                   | [ ]  |      |
| S3-3 | 세션 일시정지(수동+화면꺼짐/백그라운드 통합, 2026-07-26 확정) | [ ]  |      |
| S3-4 | 세션 심플 모드                                                | [ ]  |      |
| S3-5 | 세션 가로 프리뷰                                              | [ ]  |      |
| S3-6 | 세션 가로 심플                                                | [ ]  |      |
| S3-7 | 세션 종료 확인                                                | [ ]  |      |
...
`apps/web`은 현재 `HomePage`(소개)·`RoomPage`(임시 검증용, 디자인 미적용)만 제공한다. 스터디룸 화면은 위 표 기준으로 이 앱에 구현되어 모바일 WebView가 로드한다. 독립 브라우저 서비스로도 그대로 접근 가능하다.

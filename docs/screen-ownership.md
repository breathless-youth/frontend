...
`apps/web`은 현재 `HomePage`(소개)·`RoomPage`(S3-1/S3-2 확정 디자인 적용, 나머지 세션 상태는 WG2~WG4가 이어서 확장)를 제공한다. 스터디룸 화면은 위 표 기준으로 이 앱에 구현되어 모바일 WebView가 로드한다. 독립 브라우저 서비스로도 그대로 접근 가능하다.

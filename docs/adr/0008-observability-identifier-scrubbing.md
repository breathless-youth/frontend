...
첫 문단의 "모든 웹뷰 탭을 `?userId=N`으로 연다"는 결정 당시 설명이다. 지금 신 앱 셸은 URL에 `userId`를 싣지 않고, 토큰이 없는 구 앱 웹뷰만 `?userId=N`을 쓴다(`apps/web/src/lib/userId.ts`). 구 앱 웹뷰와 브라우저 단독 모드는 여전히 URL의 `userId`를 신원으로 읽으므로 위 정제 규칙은 그대로 유효하다.

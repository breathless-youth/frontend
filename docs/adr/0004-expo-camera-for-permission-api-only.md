...
대안이던 "WebView 계층 위임"은 `react-native-webview`가 아직 설치돼 있지 않아 지금 선택 가능한 안이 아니었다.

#### 재검토: "`getUserMedia` 하나로 권한까지 받으면 되지 않나" (2026-07-28)

`react-native-webview`가 설치된 뒤 이 대안을 다시 검토했고, **현행 유지로 결론냈다.**

기술적으로는 가능하다 — `RNCWebChromeClient.onPermissionRequest`가 `checkSelfPermission`으로 확인한 뒤 없으면 **`requestPermissions`로 OS 다이얼로그를 직접 띄운다.** 즉 `expo-camera` 없이도 권한 획득 자체는 된다(위 서술 중 "승인을 대신해 주지 않는다"는 부정확했다).

그럼에도 게이트를 네이티브에 두는 이유는 **획득이 아니라 조회와 분기** 때문이다.

- **`expo-camera` 의존성이 줄지 않는다.** 설정 화면(S6)의 권한 토글과 S2-3 분기가 권한 *상태 읽기*를 요구하는데, `getUserMedia`로 상태를 읽으려면 카메라를 실제로 켜야 한다. 조회 API는 어차피 남는다.
- **`denied` 상태에서 세션에 진입해 버린다.** 현행 게이트는 거부 이력이 있으면 세션에 들어가지 않고 S2-3으로 보낸다. WebView에 맡기면 세션 화면이 먼저 뜬 뒤 `getUserMedia`가 조용히 실패해 목업으로 떨어지고, 사용자는 "설정 열기"라는 행동 경로를 받지 못한다 — `policies.md` §3("카메라 권한 없이 사용 불가")과 어긋난다.
- **fail-closed가 사라진다.** 조회 실패 시 세션을 시작하지 않는다는 현행 동작(`cameraPermissionGate.ts`)이 "실패해도 시작됨"으로 바뀐다.
- **수동 타이머 모드 분기점이 없어진다.** `app-review-checklist.md` 1-1(스토어 심사 최우선)이 요구하는 거부 시 분기는 네이티브가 거부를 알아야 하는데, 세션 안에서 알게 되면 웹이 브리지로 되짚어 보고해야 한다. 그 메시지는 아직 없다.

**두 계층은 중복이 아니라 직렬이다.** 게이트가 먼저 받아두면 WebView 차례에는 `checkSelfPermission`이 이미 `GRANTED`라 조용히 통과한다 — 2026-07-28 Android 실측에서 프롬프트가 한 번만 뜬 것이 이 구조가 의도대로 맞물린 결과다.

**iOS는 이 재검토의 근거가 없다.** 라우트가 주는 `mediaCapturePermissionGrantType="grant"`는 WKWebView 레벨 승인이지 OS 권한이 아니며, 그것이 OS 프롬프트와 어떻게 맞물리는지 확인하지 못했다(시뮬레이터에 카메라가 없다). 실기기 확인 결과가 예상과 다르면 이 결론도 다시 봐야 한다.

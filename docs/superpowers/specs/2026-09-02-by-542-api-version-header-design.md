...
>
> 이 문서가 미뤄 둔 "v2 헤더 전환"(범위 밖 3번)은 BY-675가 했는데, 전역 축으로 처리해 갱신 요청이 400을 받는 결함이 났다. BY-723(#192)이 그 전역 기본값을 걷어내고 `packages/types`의 `API_ENDPOINTS` 레지스트리 + `apiVersionFor(endpoint, legacy)`로 엔드포인트마다 버전을 명시하게 고쳤다. 현재 규칙과 엔드포인트별 표는 [BY-509 설계 문서](./2026-09-08-BY-509-guest-token-auth-design.md)의 `API-Version` 절에 있다.
>

...
> **2026-09-21 갱신.** 발열·배터리 조정으로 프레임 주기를 500ms에서 1000ms(1 fps)로 늘리면서 `away` 확정 1.5초→**2초**, `phone` 확정 0.5초→**1초**로 올렸다. 주기와 `phone` 확정 유지시간은 짝이라 따로 바꾸지 않는다 — 근거는 `apps/web/src/features/study-session/vision/visionConfig.ts`의 `FRAME_INTERVAL_MS` 주석.

...
영상을 `:paused`에서 투명하게 하는 마스크도 시도했지만 숨겨지지 않는 것을 실기기에서
확인했고, 상대 영상이 세션 중간에 멈추면 마지막 프레임 대신 타일이 통째로 사라지는
부작용이 있어 걷어냈다(2026-08-26 리뷰 반영).
...
- 영상 탭 통과: 영상 위 탭이 컨트롤 바 복귀로 전달되고, 네이티브 버튼을 눌러도 스트림이
  멈추지 않음 — 통과
- 저전력 모드 회귀 없음 — 통과
- 네이티브 컨트롤: `play()` 킥·재시도만으로는 저전력 모드에서 1~2초 깜빡임이 남았고,
  `autoplay` 속성 제거 후 완전히 사라짐 — 통과 (재생도 정상)

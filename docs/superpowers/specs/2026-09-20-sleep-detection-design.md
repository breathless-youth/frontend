...
꾸벅거리는 사람은 한 번에 3초쯤 감았다 뜨기를 반복해 `SLEEP_EYES`의 연속 10초를 영영 못 채운다. 사이사이 뜬 눈이 유지시간을 계속 0으로 되돌리기 때문이다. 같은 1분을 합쳐서 보면 절반 넘게 감겨 있으므로, 연속이 아니라 비율로 한 번 더 본다. 이것이 PERCLOS이고, 1994년 정의부터 1분 창이 표준이다. 새 원신호 `SLEEP_DROWSY`로 나가 기존 유지시간 모듈이 받는다.
...
창에는 나이 제한이 없다. 새 표본이 안 들어오면 마지막 1분이 그대로 얼어 판정이 고정되므로, 얼굴 틱이 돌았는데 눈 판정이 `FACE_SMOOTHING_SAMPLES`만큼 연속으로 걸러지면 창을 버린다. 연속 규칙이 3표본 중앙값으로 한 표본을 거르는 것과 같은 크기라 근거가 대칭이다.
...
대가는 꼬리다. 깨어난 뒤 비율이 절반 아래로 내려오기까지 약 30초가 걸린다. 유지시간 진입을 4초로 짧게 둔 것은 원신호가 이미 1분 창으로 평활돼 있어 여기서 또 오래 기다리면 판정이 그만큼 늦기 때문이다.
...
| `EYE_RATIO_WINDOW_SAMPLES`   | 30                 | 문헌. PERCLOS 1분 창 = 얼굴 틱 2초 × 30                                                                                                                             |
| `EYE_RATIO_THRESHOLD`        | 0.5                | 문헌 + 판단. 0.15는 피로이지 잠이 아니다. 0.7이 더 보수적인 대안                                                                                                    |
| `SLEEP_DROWSY` 유지시간      | 진입 4초, 해제 3초 | 판단. 원신호가 이미 평활돼 있다. 해제는 다른 졸음 출처와 같다                                                                                                       |
...
`EYE_THRESHOLD_MIN`은 고정 임계와 같은 0.45다. **보정은 임계를 올리기만 하고 내리지는 못한다.** 내릴 수 있게 두면 보정이 판정을 느슨하게 만드는 방향이 열리는데, 그쪽은 깨어 있는 사람을 졸음으로 잡는 오탐이다. 눈이 큰 사람은 감아도 점수가 낮아 고정 임계에 못 미칠 수 있고 그 사람에게는 하한을 내리는 것이 맞지만, 그 데이터가 실기기에서 나오기 전까지는 오탐을 막는 쪽으로 둔다. 실기기 A17이 그 방향을 재고, 눈 큰 사람의 감김 분포가 나오면 하한을 다시 정한다.
...
**창이 개수 기반이라 1분은 벽시계 1분이 아니다.** 표본 30개를 세는 것이라, 프레임이 버려지거나 얼굴 틱이 건너뛰어지면 그 30개가 덮는 실제 시간이 1분보다 길어진다. 발열로 추론이 느려질수록 창이 조용히 늘어나고, 그만큼 판정이 늦고 깨어난 뒤 꼬리도 길어진다. 실기기에서 `dropped`와 함께 본다.

**창을 버리면 다시 서기까지 30개가 새로 필요하다.** 자리 이탈·일시정지·얼굴 모델 사망·눈 판정 연속 누락·임계 하락에서 창을 버리는데, 그 뒤 `SLEEP_DROWSY`가 다시 서려면 깨끗한 표본 30개, 즉 1분이 또 걸린다. 자리를 자주 비우는 사람에게는 비율 판정이 사실상 안 도는 구간이 생긴다. 오탐을 막는 쪽으로 택한 대가이고, 연속 규칙(`SLEEP_EYES`)은 그 구간에서도 계속 돈다.

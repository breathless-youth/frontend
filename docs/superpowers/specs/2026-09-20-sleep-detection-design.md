...
**측정 도구에 더한 것.** 구간별 `headPitch`·`ear` 분포와 `skipped["looking-down"]`. EAR(눈 종횡비)은 판정에 쓰지 않고 blendshape와 비교하기 위해서만 남긴다. 게이트 값과 자세 행렬의 부호는 다음 회차에서 확정한다(당시 런북 기준. 런북은 측정 도구와 함께 지웠다).
...
### 화소 대비도 실패, 전부 뺀다 (2026-09-22, BY-704 넷째 회차)

같은 자세(고개 17~~19°)의 `내려다봄`과 `숙이고 감기`에서 눈 영역 밝기 표준편차는 0.078 vs 0.084, 어두운 화소 비율은 0.014 vs 0.018로 같았다. 정면에서는 어둠 비율이 뜸 0.04~~0.05 vs 감음 0.017로 갈렸지만, 내려다보면 뜬 눈의 동공이 윗눈꺼풀 뒤로 들어가 카메라에 실제로 보이지 않는다. 랜드마크가 틀린 것이 아니라 카메라가 못 보는 것이라 눈에서 나오는 신호로는 어떤 것도 못 가른다. EAR·내려다봄 점수·화소 지표와 그 계측을 전부 뺐다. 고개 각도는 자세 해석용 참고값으로만 남긴다.

같은 회차에서 `내려다봄`(책, 19°)은 눈 점수 0.233·EAR 0.307로 뜬 눈으로 잘 읽혔다. 첫 회차의 휴대폰(0.554)·둘째 회차의 내려다봄(EAR 0.085)과 다른 점은 시선이 카메라보다 얼마나 가파르게 떨어지느냐다. 오탐은 "카메라가 눈 위에 있고 시선이 그보다 훨씬 아래로 가는" 자세에서만 난다.

우선순위 재확인(리더): 오탐 0이 목표이고, 내려다볼 때 졸음으로 찍히지 않는 것이 숙이고 자는 것을 잡는 것보다 중요하다. 얼굴 검출·존재 신뢰도는 0.6→0.5로 내렸다(숙인 자세에서 얼굴이 30번 중 10번 빠져 유지시간이 끊겼다).

### 시선 이동 vs 감김 — 감김의 시작 순간으로 가른다 (2026-09-22, 구현)

**근거.** 미국 특허 11144756(시선 이동과 눈 감김의 구분)의 관찰: 아래를 보는 시선 이동에는 고개 각도 변화가 따라오지만, 졸음으로 인한 눈 감김에는 따라오지 않는다. 특허는 감김 시작 전후 2초의 pitch 변화 ±5°로 가른다. 우리 실측과 맞는다 — 앉아서 감는 동안 고개는 2° 안에서 흔들렸고, 책·폰을 보려고 내릴 때는 한 틱(2초)에 15~20° 움직였다.

**규칙(`vision/glanceRule.ts`, 순수 상태기계).** 얼굴 틱마다 눈 점수(두 눈 중 덜 감긴 쪽)와 고개 각도를 넣고, 임계는 판정 시점에 댄다(보정 전 관측도 기록해 임계가 생기자마자 쓴다).

- **쉬는 자세(`rest`)** = 뜬 눈으로 고개를 멈춘 채 `GLANCE_REST_OPEN_TICKS`(5틱 = 10초) 이어진 각도. 앉아서 눈을 뜨고 있으면 바로 성립하고, 책을 읽는 동안은 눈 점수가 임계 근처에서 흔들려 뜸이 10초 연속되지 않으므로 읽는 자세는 쉬는 자세로 잡히지 않는다.
- 감김 시작 틱에서 쉬는 자세 대비 |Δpitch| ≥ `GLANCE_HEAD_MOVE_DEG`(5°)면 **시선 이동 잠금**. 아니면 `pending`으로 두고 다음 틱에서 시작 각도 대비 한 번 더 본다(앞뒤 한 틱 창).
- 잠금은 눈이 잠깐 뜸으로 읽혀도 풀리지 않는다. 고개가 쉬는 자세 근처(5° 안)로 돌아온 채 뜨거나, 그 자리에서 뜬 눈이 10초 이어져 새 쉬는 자세가 잡힐 때만 풀린다. 첫 구현은 뜸 하나로 풀었고, 넷째 회차 재측정에서 읽는 동안 흔들리는 점수의 뜸 하나가 들어오자 다음 감김이 "고개 정지 · 새 시작"으로 통과해 22초에 졸음이 섰다(`내려다봄` 구간 눈 점수 0.16~0.65, glance 3건).
- 쉬는 자세를 모르는 감김(세션 시작부터 감김, 뜬 눈 10초가 없었음)은 받아들이지 않는다. 임계가 바뀌면 기록된 점수를 새 임계로 다시 읽으므로 별도 처리가 없다.
- 시선 이동으로 분류된 관측은 눈을 지워 `eyeSkipReason: "glance"` — "판정 없음"이지 "눈 뜸"이 아니다. 평활·비율·보정 창 어디에도 들어가지 않는다.
- 각도를 모르면(자세 행렬 없음) 감김으로 둔다. 상태는 얼굴 관측과 수명이 같다(`dropFaceObservations`).

**절대 각도는 보지 않는다.** 그래서 카메라 위치에 따라 공부 자세가 15°든 25°든 상관없다 — 거기서 고개를 멈춘 채 시작된 감김만 감김이다. 2026-09-22에 잠시 넣었던 절대 각도 게이트가 폰을 낮게 두면 정상 판정까지 끊던 문제가 없다.

**놓치는 것(리더 결정, 오탐 0이 우선).** 고개를 내리면서 시작된 감김 전부 — 숙인 채 자는 것, 고개를 떨구며 꾸벅거리는 것. 남는 오탐 가능성은 고개는 그대로 두고 눈만 크게 내리는 경우뿐이고, 특허도 같은 한계를 둔다(Euro NCAP의 "lizard" 시선).

**검증 시나리오(BY-704).** 폰은 정면·살짝 위. ① 앉아서 눈 감기 → 졸음. ② 고개 내리며 책 보기 → `(시선 이동)`, 졸음 없음. ③ 그 자세 유지 → 졸음 없음. ④ 고개 들고 정면 → 뜸으로 복귀. ⑤ 다시 정지 감김 → 졸음. ⑥ 손에 든 폰 보기 → 졸음 없음.

### 확정: 절대 각도 게이트 (2026-09-22, BY-704 6~9회차 뒤)

**위 "시선 이동 vs 감김" 절과 그 후속 수정(쉬는 자세·잠금·계속·급강하)은 이 절로 대체된다.** 특허 방식의 원리는 맞지만 **눈을 감은 표본의 고개 각도는 믿을 수 없었다** — 감은 동안 5회차 ±1.5°, 8회차 최대 23°, 9회차 -23~+18°. 시작 순간의 각도를 보는 규칙은 그 흔들림을 시선 이동으로 읽어 진짜 감김을 잠갔고, 회차마다 다른 데서 깨졌다. 리더: "이 환경에 맞추는 수정이 아니라 모두에게 오탐 0이어야 한다. 조는 걸 못 잡는 건 괜찮다."

**규칙(`vision/headPitchGate.ts`, 순수 함수).** 카메라 기준 고개 각도의 최근 3틱 중앙값이 `HEAD_PITCH_DOWN_DEG`(15°) 이상이면 그 관측의 눈을 지워 `eyeSkipReason: "looking-down"`. 판정 없음은 원신호를 내리므로(`eyesClosed`는 관측이 없으면 false) 서 있던 졸음은 2초 뒤 풀리고 새로 서지 않는다 — 순공으로 세는 쪽이다. 상태는 최근 각도 3개뿐이고 얼굴 관측과 수명이 같다.

**왜 절대 각도인가.** 오탐 조건은 하나 — 카메라가 시선보다 위에 있어 윗눈꺼풀이 눈을 가려 보이는 것 — 이고 그것이 곧 카메라 기준 각도다. 아래에서 찍으면 음수라 감지 구간이고 눈이 보이니 감김도 정확하다. 정면은 책·폰을 볼 때 18° 이상. 위에서 찍으면 거의 항상 게이트에 걸려 감지하지 않는다(합의된 포기). 앉아서 감을 때 중앙값은 0~5°라 15°는 양쪽에 여유가 있다.

**빼기로 한 것.** 가운데 구간(15~~20°)의 "30초 무깜빡임 = 잠" 규칙. 2초 표본으로는 깜빡임이 안 잡히고(틱당 5~~15%), 초당 5~~10번 보면 발열이 27~~54%p 늘며, 집중해 읽을 때 30초 넘게 안 깜빡이는 것이 흔해 오탐 0이 안 된다.

**놓치는 것.** 감은 채 고개가 15° 넘게 떨어진 뒤, 각도 추정이 흔들리는 세션, 위에서 찍는 배치. 전부 "안 세는" 방향이다.

### 깜빡임 계측 — 1단계 (2026-09-22)

리더 제안: 게이트 구간(15° 이상)에서 **깜빡임이 이어지면 깨어 있는 것이고, 같은 각도에서 깜빡임이 끊기면 잠**으로 볼 수 있다. 판정은 아직 넣지 않고 신호가 잡히는지부터 잰다.

- 얼굴 모델을 초당 10번 돌리면 CPU 듀티가 54%p 늘어 발열을 잡던 방향과 반대라, 모델 없이 **랜드마크가 잡아 둔 눈 자리(2초 갱신)의 화소만 초당 10번 비교**한다(`blinkWatcher.ts`). 48×24 두 조각의 뺄셈이라 비용은 사실상 없다. 게이트에 걸린 동안만 돈다.
- 표본마다 평균 절대 변화(`diff`)와 잠정 이벤트 여부를 남긴다. 이벤트 = 배경 변화의 지수이동평균 × `BLINK_DIFF_RATIO`(4)와 `BLINK_DIFF_MIN`(0.03) 중 큰 쪽 이상, 그 뒤 400ms 불응기. 덩어리 구간에 `blink.samples/events/diffP50/diffP95/diffMax`.
- 앞서 눈 영역의 **정적** 대비·어둠 비율은 실패했다(넷째 회차). 이번 것은 눈꺼풀이 움직이는 **순간 변화**라 다른 신호이고, 검증은 이 계측이 한다.
- 검증 시나리오: 같은 각도(18°쯤)에서 `내려다봄` 2분과 `숙이고 감기` 2분. 읽을 때 1~2분마다 이벤트가 최소 1회 잡히고 감았을 때 0이면 성립. 성립하면 2단계 규칙: 게이트 구간에서 감김이 N초(≥120초, 데이터로) 이어지고 그 사이 이벤트 0이면 졸음. 30초는 안 된다 — 집중해 읽을 때 30초 넘게 안 깜빡이는 것이 흔하다.
- 화소는 그 자리에서 숫자 하나로 줄고 버려진다.

### 확정: 세 구간 (2026-09-22, BY-704 열째 회차 뒤)

열째 회차에서 책을 읽는 자세의 고개 각도가 중앙값 15.2°(하위 5% 3.3°)로 15° 게이트 바로 위에 걸쳐 표본 절반이 판정에 들어갔고 76초에 졸음이 섰다(오탐). 앉아서 감을 때는 중앙값 0.4°(상위 5% 14°)라 각도 하나로는 겹치는 구간을 못 가른다. 같은 회차에서 읽는 동안 눈 움직임 계측이 분당 17회(깜빡임 + 시선 이동)를 잡았다. 리더 결정:

| 최근 3틱 각도 중앙값        | 동작                                                                                                                                          |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| 10° 미만 (`clear`)          | 지금처럼 감김 → 10초 → 졸음                                                                                                                   |
| 10° 이상 15° 미만 (`quiet`) | 눈 움직임 계측이 돌고 최근 `BLINK_QUIET_MS`(60초) 동안 이벤트가 0일 때만 감김을 셈. 아니면 `eyes-active`(판정 없음). 계측이 안 돌면 판정 없음 |
| 15° 이상 (`down`)           | `looking-down`(판정 없음)                                                                                                                     |

읽는 자세의 하위 5%가 3.3°라 10° 아래로 내려오는 표본은 5% 미만이고, 10초 유지가 그것만으로 채워질 확률은 사실상 0이다. 60초는 읽는 동안 무이벤트가 사실상 안 나오는 길이이고, 감은 눈에서 이벤트가 0에 가깝다는 것이 `숙이고 감기` 측정으로 확인되면 줄인다. 아래에서 찍는 배치는 각도가 음수라 `clear`이고 눈이 보이니 감김도 정확하다.

**남는 오탐 경로.** 10° 미만에서 고개는 두고 눈만 크게 내려 보는 경우(5회차 휴대폰: 고개 -4°, 눈 0.675). 각도로는 못 막고, 이 구간에도 눈 움직임 조건을 두는 것이 다음 후보다 — 감은 눈의 이벤트 분포가 먼저다.

### 정지는 수준으로 잰다 (2026-09-22, 열둘째 회차)

튀는 표본(이벤트)을 세고 "60초 무이벤트"를 정지로 보던 규칙이 두 방향 모두 틀렸다. 읽는 동안(고개 18.6°)은 눈이 계속 움직여 배경 변화가 0.022로 올라가고, 배경 대비 4배로 문턱을 잡던 규칙이 문턱을 상위 5%(0.0547) 위로 밀어 **이벤트가 0**이 됐다 — 깨어 있다는 증거가 증거를 지웠다. 감은 눈(11.4°)은 배경이 0.0047인데 머리 흔들림 두 번(최대 0.116)이 이벤트로 잡혀 정지가 서지 않았다.

두 회차 다 변화량의 **중앙값**은 읽을 때 0.0133·0.0222, 감았을 때 0.0037·0.0047로 3~5배 갈리고 튐에 흔들리지 않는다. 규칙: 최근 `EYE_MOTION_WINDOW_MS`(10초) 변화량의 중앙값이 `EYE_MOTION_LEVEL`(0.009) 이상이면 움직임이고 정지 시계를 되돌린다. `quiet` 구간의 감김은 정지가 `BLINK_QUIET_MS`(60초) 이어졌을 때만 센다. 이벤트 카운트(`BLINK_DIFF_MIN` 0.04, 리더 결정)는 참고용으로만 남긴다. 낮을수록 오탐이 준다 — 문턱을 올리면 읽는 사람이 정지로 읽힌다.

### 객체 추론 2 fps 복귀 (2026-09-22 저녁)

리더 결정으로 `FRAME_INTERVAL_MS`를 1000에서 500으로 되돌렸다. 졸음 측정 회차에서 객체 추론이 124ms라 500ms 주기에서도 듀티 25%로 여유가 있고, 이탈·휴대폰의 반응성을 되찾는 쪽을 골랐다. 얼굴 틱은 2초 그대로(`FACE_FRAME_DIVISOR` 2→4), 유지시간은 500ms 시절 값(AWAY 1500 · PHONE 500)으로 되돌렸다. 발열은 `?sleep=0` A/B로 다시 잰다.

## 확정 (2026-09-22 밤, BY-704 열셋째 회차 뒤)

**위의 "세 구간"·"깜빡임 계측"·"정지는 수준으로 잰다" 절은 이 절로 대체된다.** 리더 결정: 오탐(안 자는데 잔다고 함) 0이 우선이고, 조는 걸 놓치는 것은 허용한다.

| 규칙            | 값                                                                                   |
| --------------- | ------------------------------------------------------------------------------------ |
| 내려다봄 게이트 | 카메라 기준 고개 각도 최근 3틱 중앙값 ≥ `HEAD_PITCH_DOWN_DEG`(**10°**)이면 판정 없음 |
| 눈 감김(연속)   | 감김 10초 → 졸음, 뜸 2초 → 해제 (`SLEEP_EYES`)                                       |
| 꾸벅거림(비율)  | 44초 창에서 감김 ≥ 50% → 4초 → 졸음, 뜬 눈 6초 연속이면 창 비움 (`SLEEP_DROWSY`)     |
| 눈 임계         | 30초 보정한 뜬 눈 기준 + 0.25, 창마다 재고 최솟값                                    |
| 얼굴 틱         | 2초(`FACE_FRAME_DIVISOR` 2 × 1000ms)                                                 |
| 객체 추론       | 1 fps, 이탈 2초 · 휴대폰 1초, person 0.3 · 휴대폰 0.4                                |

**게이트가 10°인 이유.** 앉아서 감을 때 각도 중앙값은 0~~5°, 읽는 자세는 하위 5%가 3.3°·중앙값 11.6~~18.6°였다. 15°에서는 열째 회차처럼 읽는 자세가 게이트에 걸쳐 76초에 졸음이 섰다(오탐). 10°면 읽는 동안 판정에 들어가는 표본이 5% 미만이라 10초 유지가 채워지지 않는다.

**뺀 것과 이유.**

- 10~~15° 구간의 눈 움직임 조건. 열셋째 회차에서 읽을 때 0.0062, 감았을 때 0.0041로 차이가 1.5배뿐이고, 앞 회차의 3~~5배는 상자 교체 튐이 부풀린 값이었다. 화소 변화량은 조명·거리·눈 크기로 회차마다 두 배씩 움직여 어떤 고정 문턱도 어느 회차에서는 오탐 쪽으로 넘어간다. 사람별 보정 없이는 못 쓴다. 계측 코드(`blinkWatcher.ts`, 눈 자리 화소를 초당 10번 `drawImage`로 읽던 것)도 확정 뒤 지웠다 — 판정에 안 쓰는 계측을 발열 예산에 남길 이유가 없고, 발열 A/B에서 졸음 켬이 끔의 53%로 떨어진 유력한 원인이기도 하다.
- 시선 이동 상태기계(특허 방식). 눈을 감은 표본의 고개 각도가 흔들려(-23~+23°) 진짜 감김을 잠갔다.
- EAR·내려다봄 점수·화소 대비. 카메라가 눈을 위에서 보면 동공이 눈꺼풀 뒤로 들어가 어느 것도 못 가른다.

**남는 것.** 오탐 경로는 고개는 10° 아래에 두고 눈만 크게 내려 보는 경우(5회차 휴대폰: 고개 -4°, 눈 0.675)뿐이다. 놓치는 것은 고개를 10° 넘게 숙인 채 자는 것, 위에서 찍는 배치, 각도 추정이 흔들리는 세션이다. 아래에서 찍는 배치는 각도가 음수라 감지 구간이고 눈이 보이니 감김도 정확하다 — 온보딩에 "폰은 눈높이나 그 아래"를 넣는 것이 남은 오탐 경로까지 줄인다(리더 확인 항목).

**객체 추론 주기.** 2026-09-22 저녁 한 회차만 500ms(2 fps)·이탈 1.5초·휴대폰 0.5초로 돌려 봤고, 졸음 측정용 임시 변경이라 1000ms(1 fps)·이탈 2초·휴대폰 1초·얼굴 2프레임(2초)으로 되돌렸다(리더). 발열 A/B는 이 값으로 잰다.

## 발열 A/B 결과 (2026-09-22 밤, 리더 실기기 · Safari · 터널)

같은 폰, 같은 자리, 1 fps · 얼굴 2초. 기준은 켬의 Serious 도달 시간이 끔의 80% 이상.

| 회차                             | Fair | Serious | 배터리       | 끔 대비 |
| -------------------------------- | ---- | ------- | ------------ | ------- |
| 졸음 끔(`?sleep=0`)              | 12분 | 34:15   | 15% / 34분   | 100%    |
| 졸음 켬, 눈 움직임 계측 있음     | 6분  | 18:00   | 12% / 18분   | 53%     |
| 졸음 켬, 계측 지운 뒤(`22d298b`) | 11분 | 27:35   | 13% / 27.5분 | 80.5%   |

**읽기.** 얼굴 모델 자체의 몫은 Serious까지 약 7분(20%), 배터리 소모율로는 분당 0.44% → 0.47%(+7%)다. 계측을 지우기 전의 18분은 얼굴 모델이 아니라 게이트에 걸린 동안 초당 10번 돌던 `drawImage(video)`(iOS에서 전체 프레임 GPU→CPU 복사) 때문이었다. 켬 회차는 80.5%로 합격선을 아슬아슬하게 넘었다 — 한 회차 실측이라 여유는 없고, 기준을 더 조이려면 `FACE_FRAME_DIVISOR`를 3(얼굴 3초)으로 올리는 것이 다음 손잡이다. 지금은 2초를 유지한다(10초 유지시간에 표본 5개, 3초면 3~4개라 한 표본 튐의 무게가 커진다).

...
- 10~~15° 구간의 눈 움직임 조건. 열셋째 회차에서 읽을 때 0.0062, 감았을 때 0.0041로 차이가 1.5배뿐이고, 앞 회차의 3~~5배는 상자 교체 튐이 부풀린 값이었다. 화소 변화량은 조명·거리·눈 크기로 회차마다 두 배씩 움직여 어떤 고정 문턱도 어느 회차에서는 오탐 쪽으로 넘어간다. 사람별 보정 없이는 못 쓴다. 계측(`blinkWatcher.ts`)은 덩어리 기록용으로만 남긴다.

...
**규칙(`vision/glanceRule.ts`, 순수 상태기계).** 얼굴 틱마다 눈이 임계 이상인지(`closed`)와 고개 각도를 넣는다.

- 뜸→감김 전이 틱에서 직전 틱 대비 |Δpitch| ≥ `GLANCE_HEAD_MOVE_DEG`(5°)면 그 감김 구간은 **시선 이동**. 아니면 `pending`으로 두고 다음 틱에서 시작 각도 대비 한 번 더 본다(앞뒤 한 틱 창).
- 시선 이동 구간의 관측은 눈을 지워 `eyeSkipReason: "glance"`로 바꾼다 — "판정 없음"이지 "눈 뜸"이 아니다. 평활·비율·보정 창 어디에도 들어가지 않는다. 눈이 다시 뜸으로 읽히면 구간이 닫힌다.
- 지금 임계에서 **뜬 눈을 본 적이 없는 감김은 받아들이지 않는다** — 세션 시작부터 감김이거나, 보정으로 임계가 바뀌어 감김이 된 경우는 시작 순간이 없다(`resetGlanceForThreshold`).
- 판정 없는 틱(얼굴 없음·게이트) 뒤의 감김은 새 시작으로 보고, 마지막으로 안 각도와 비교한다 — 얼굴이 없던 사이 고개가 내려간 것이다.
- 각도를 모르면(자세 행렬 없음) 감김으로 둔다. 규칙이 없던 때의 동작이고, 행렬은 프로덕션에서 켜져 있다.
- 상태는 얼굴 관측과 수명이 같다(`dropFaceObservations`).

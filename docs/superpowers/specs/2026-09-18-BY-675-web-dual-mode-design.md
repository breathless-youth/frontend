...
`API-Version` 헤더는 토큰 부착 여부로 갈린다. 토큰을 붙인 요청은 `2`, 붙이지 않은 요청은 `1`을 보낸다. 호출부가 직접 지정한 값은 그대로 우선한다. 백엔드가 이 값으로 토큰 인증 요청과 구 방식 요청을 가른다(2026-09-18 요청). 모바일의 등록·갱신 요청과 소켓 CONNECT에 같은 값을 보낼지는 백엔드 확인 뒤 정한다.

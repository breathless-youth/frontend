...
> **이 규칙은 구조적으로 틀렸다(2026-09-22 확인).** 버전은 **엔드포인트 단위**라 토큰 부착 여부로 정할 값이 아니다. 이 티켓이 부르는 경로가 전부 v2여서 결과만 우연히 맞았고, v1인 새 경로(`/api/stats/study-days`, `/api/subjects`)를 붙이는 순간 400이 난다. 표와 고칠 방향은 [BY-509 설계 문서](./2026-09-08-BY-509-guest-token-auth-design.md)의 `API-Version` 절에 있다.

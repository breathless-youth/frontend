...
| 결과(S4)      | `study_result_confirmed {room_type, via: cta/close}` · `study_result_exited {room_type, focus_sec, destination: home/record}`(설문 트리거) · `study_result_distraction_toggled {status, expanded}`                                                                                                                                 | `ResultPage`·`DistractionStatsCard`. 결과 화면 노출은 페이지뷰 `/room/:id/result`·`/social/room/:id/result`가 이미 갖는다(별도 `_viewed` 없음)                                                                                                                                                       |

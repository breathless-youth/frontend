...
| 결과(S4)      | `study_result_confirmed {room_type, via: home/records}`                                                                                                                                                                                                                                                                            | `ResultPage`. `study_result_distraction_toggled`는 2026-09-14 BY-560에서 비집중 통계 카드가 요약 카드로 바뀌며 제거. `via`는 CTA `홈으로`/`기록으로 가기`(BY-560, 전에는 `cta/close`). `기록으로 가기`의 탭 전환은 네이티브 `tab_pressed {via: study_result}`. 결과 화면 노출은 페이지뷰 `/room/:id/result`·`/social/room/:id/result`가 이미 갖는다(별도 `_viewed` 없음) |

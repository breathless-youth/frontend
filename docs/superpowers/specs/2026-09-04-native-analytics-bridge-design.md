...
| `tab_pressed`                                                                               | `tab`, `from_tab` (`home/social/record/settings`), `via` (`tab_bar/card/study_result/hardware_back`)             | `TabBar`(활성 탭은 비활성화라 재터치 없음) + `nativeBridgeHandler`의 `navigate-tab`(홈 연속 공부 카드 → 기록, 출발 탭은 `lib/activeTab.ts`) + `app/(tabs)/_layout.tsx`의 Android 뒤로가기 홈 복귀 — 사용자에겐 전부 같은 탭 이동이라 한 이벤트, 경로만 `via`         |
...
| 결과(S4)      | `study_result_confirmed {room_type, via: home/records}` · `study_result_exited {room_type, focus_sec, destination: home/record}`(설문 트리거 — 결과 화면은 localStorage에 예약만 하고 홈·소셜·기록 도착 시 `AnalyticsRouteTracker`가 보낸다: 세션 모달 웹뷰가 닫혀도 살아남게)                                                     | `ResultPage`. `study_result_distraction_toggled`는 BY-560에서 비집중 통계 카드가 요약 카드로 바뀌며 제거. `via`는 CTA `홈으로`/`기록으로 가기`(전에는 `cta/close`). `기록으로 가기`의 탭 전환은 네이티브 `tab_pressed {via: study_result}`. 결과 화면 노출은 페이지뷰 `/room/:id/result`·`/social/room/:id/result`가 이미 갖는다(별도 `_viewed` 없음) |

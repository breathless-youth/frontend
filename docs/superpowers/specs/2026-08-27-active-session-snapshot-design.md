...
- `400`과 `404`는 다시 보내도 같은 결과라 그 세션 동안 보고를 멈추고 Sentry에 한 번 남긴다.
- `409`는 계속 보고하고 Sentry에 한 번만 남긴다. 일시적 경합이면 이후 세션을 잃지 않고, 지속적이어도 최악은 무해한 반복이다.
...
- `PUT /api/study-sessions/active`의 `409`가 어떤 상황인지 확인한다. 일시적 경합이면 계속 보고가 맞고, 세션이 이미 확정된 상태면 보고 중단으로 바꾼다.
- 최종 제출은 10분 미만 세션을 저장하지 않는데 자동 확정은 길이를 따지지 않는다. 10분 미만 세션이 자동 확정으로만 저장되는 역전이 생기는지 확인한다.

...
**버전은 서버 전체가 아니라 엔드포인트 단위다.** 각 API가 만들어질 때 `1`에서 시작하고 그 엔드포인트의 계약이 바뀔 때마다 오른다. 그래서 이 헤더는 "구 방식이냐 신 방식이냐"가 아니라 **"이 엔드포인트의 몇 번째 계약을 원하느냐"**를 말한다.

판별 규칙은 하나다. **구 앱(v1.2.x)이 부르던 경로만 `2`**(토큰 도입으로 계약이 바뀐 것), **그 외 새로 생긴 경로는 전부 `1`**이고 앞으로 추가되는 API도 `1`이다. 원본은 Swagger의 각 API `API-Version` 파라미터 기본값이다.

| 버전 | 엔드포인트                                                                                                                                                                                                                                                                                                                                                                  |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `2`  | `POST /api/users` · `GET`·`PATCH /api/users/me/profile` · `POST /api/rooms` · `POST /api/rooms/join` · `POST /api/rooms/{roomId}/leave` · `POST /api/study-sessions` · `GET /api/study-sessions/{id}` · `PUT`·`GET /api/study-sessions/active` · `POST /api/study-sessions/recovery` · `GET /api/stats` · `/api/stats/streak` · `/api/stats/period` · `POST /api/rtc-stats` |
| `1`  | `POST /api/auth/refresh` · `GET /api/stats/study-days` · `/api/subjects` CRUD와 하위 tasks                                                                                                                                                                                                                                                                                  |

- **다른 값을 보내면 400이다.** 양방향으로 엄격하다 — v2 경로에 `1`을 보내도, v1 경로에 `2`를 보내도 거절된다(2026-09-22 개발 API 실측).
- `POST /api/auth/refresh`가 `1`인 이유는 규칙 그대로다. 구 앱에는 토큰이 없어 갱신을 부른 적이 없으니 새 경로다.
- **그래서 공통 래퍼에 전역 기본값을 두면 안 된다.** 각 API 함수가 자기 엔드포인트의 버전을 명시한다. 기본값이 없으면 새로 붙이는 v1 경로는 헤더 없이도 맞고, v2 경로는 빠뜨리는 순간 400으로 즉시 드러난다.
- 우리가 오늘 부르는 경로는 전부 v2라 옛 "토큰 있으면 2/없으면 1" 규칙도 결과만은 맞았지만, `study-days`나 `subjects`를 붙이는 순간 400이었다. BY-723(#192)이 `packages/types`의 `API_ENDPOINTS` 레지스트리 + `apiVersionFor(endpoint, legacy)`로 바꿔, 호출부가 엔드포인트 키를 넘기면 그 엔드포인트의 버전이 나간다. `study-days`도 이제 v1로 나가고, 값을 빠뜨리거나 잘못 넣으면 타입·단위 테스트에서 400이 사용자에게 닿기 전에 걸린다.

### 등록 응답에 `userId`가 없다 (2026-09-22 실측)

v2 등록 응답은 `{accessToken, isNew, refreshToken}`이다. **`userId`는 access JWT의 `sub`에 들어 있고 문자열이다**(`sub: "260"`). 2026-09-18 검증 때는 응답에 `userId`가 있었으므로 그 뒤 바뀐 계약이고, 그때 통과시킨 검증은 이 지점에 한해 더는 유효하지 않다.

...
홈(S1) 위에 딤과 함께 올라오는 바텀 시트다. "다음 업데이트에 로그인이 추가되고, 지금까지의 기록은 로그인하면 계정에 그대로 이어진다"는 사실을 미리 알려, V1.2 로그인 도입 시점에 익명 기기 계정 사용자가 기록 손실을 걱정하지 않게 하는 것이 목적이다(계정 병합 정책 근거: `.ai/product/policies.md` §2).

**V1.0에는 이 시트를 "개발만" 한다 — 기본은 노출되지 않는다.** 노출 시점(예: V1.2 로그인 출시 임박)이 오면 외부 설정값만 켜서 활성화한다(`.ai/product/design.md` "업데이트 안내" 행, 2026-07-26 6차 확정). 따라서 이 화면의 구현 성공 기준은 "홈에서 보인다"가 아니라 **"기본 상태에서 절대 보이지 않고, 플래그를 켰을 때만 Figma대로 보인다"**이다.
...
- .ai 근거 문서:
  - `.ai/product/design.md` — "V1.0 최종 확정" 표의 **업데이트 안내** 행, "화면 인벤토리 (V1.0 최종)" U1 항목, 백로그 1번
  - `.ai/product/user-flow.md` — 화면 목록 U1 행("홈 바텀 시트 1회. V1.0에 개발, 기본 비노출 — 노출 조건 제어")
  - `.ai/product/voice-tone.md` — §4 "업데이트 안내 시트 (U1)" 문구 표
  - `.ai/product/policies.md` — §2 계정·데이터(로그인 V1.2 도입 · 익명 계정 병합 · Google/Apple만)
  - `.ai/product/roadmap.md` — V1.0 범위 "U1 업데이트 시트(개발만, 조건 노출)"
  - `.ai/notes/2026-07-26-디자인-반영-인터뷰-6차.md` — U1 결정 원문
...
`.ai/product/voice-tone.md` §4 "업데이트 안내 시트 (U1)"에서 그대로 인용. Figma 텍스트와 **완전히 일치**함을 확인했다(의역·재작성 금지).
...
- `.ai` 전체에도 원격 설정(remote config) API, 버전 비교 정책, 강제/선택 업데이트 구분이 **전혀 등장하지 않는다**(`design.md`·`user-flow.md`·`policies.md`·`roadmap.md`·`voice-tone.md`·6차 노트 전수 확인).
...
- [ ] "1회 노출"의 재노출 기준 확정 — 단일 boolean인지, 공지 ID 단위인지 (앱 재설치·플래그 재활성 케이스)
  - ⚠️ 재노출을 도입하면 `apps/mobile/components/UpdateNoticeSheet.tsx`의 **열림 애니메이션도 함께 손봐야 한다.** 열림은 `onLayout` 1회 발화에 의존한다(`opened` ref). 닫힘 모션이 끝나 서브트리가 언마운트된 뒤 다시 띄우는 경로는 새 레이아웃이 발생해 정상 동작하지만, **닫힘 모션이 끝나기 전(슬라이드 320ms 이내)에 `visible`이 다시 켜지면 레이아웃이 그대로라 `onLayout`이 재발화하지 않아 슬라이드 인이 시작되지 않고 시트가 중간 위치에 멈춘다.** 단일 노출 전제에서만 성립하는 구현이다(현재는 재노출 경로 자체가 없어 도달 불가).
- [ ] **`seen` 조회 실패 시 노출 방향 확정** — 현재 구현은 fail-closed(비노출, `apps/mobile/lib/updateNotice.ts`의 `shouldShowUpdateNotice` catch)지만 스펙 미규정 상태에서 빌더가 자체 판단한 것이다. Exposure Control의 fail-closed 규정은 **플래그 파싱**에만 적용되고 저장소 조회 실패는 다루지 않았다. 확정되면 그 `catch` 하나만 바꾸면 된다(반대 방향인 `onboardingGuideStore`의 fail-open과 대비 관계).
  - 판단 시 고려: fail-open(조회 실패 시 노출)을 택하면, 조회가 실패하는 환경에서는 `markUpdateNoticeSeen` 쓰기도 실패할 가능성이 높아 **매 실행마다 홈을 가리는 모달이 뜨는데 닫아도 기억되지 않는 루프**가 된다. 현재 닫기 경로가 `확인` CTA 하나뿐이라(위 "닫기 경로 확정" 항목 미정) 사용자가 빠져나올 수단도 없다 — 즉 이 항목의 판단은 닫기 경로 확정 결과에 따라 무게가 달라진다.

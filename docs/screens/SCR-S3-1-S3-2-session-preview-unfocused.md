...
S3-1 · 집중 측정 중  (frame 58:323, 402×874)
  ...
  Session / Control Bar         (58:361)  79,756 244×80   ← 컴포넌트 34:32
    ...
    btn/pause       50×50 원형 bg rgba(255,255,255,0.12) · icon/pause 16×18 ⚠️ 구현은 20×22.5 (BY-336 의도 이탈, 아래 참고)
...
- [ ] **[디자인 확인] play/pause 아이콘 1.25배 확대(BY-336, 2026-07-31)** — 실기기에서 재생 아이콘이 작다는 확인으로 Figma 실측 16×18에서 **20×22.5**(가로 17.6×19.8)로 의도 이탈했다. flip·exit은 실측 유지. play SVG는 프레임 대비 잉크가 44%뿐이라(pause는 72%) 프레임 스케일로는 play/pause 글리프 불균형이 남는다 — 근본 해결은 play 아이콘을 잉크에 맞게 Figma에서 재익스포트하는 것. 디자이너 확정 시 `SessionControlBar.tsx`의 `ICON_SIZE.pause` 세 줄만 고친다.

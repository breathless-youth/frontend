...
> **⚠️ 2026-07-26 정책 대체**: 본문이 전제하는 "카메라 필수(권한 없으면 사용 불가)"는 폐기됐다 — 권한 거부 시 **수동 타이머 모드**로 전환한다(`.ai/product/policies.md` §3 측정 정책, 스토어 심사 대응). S2-3의 "측정 불가" 프레이밍·CTA는 수동 타이머 모드 도입 작업에서 재검토할 것. 아래 본문은 작성 시점(대체 이전) 기준이다.

## Purpose

FocusON은 카메라로만 측정한다(~~`.ai/product/policies.md` §3 "카메라 필수: MVP는 카메라 권한 없이 사용 불가"~~ → 2026-07-26 수동 타이머 모드로 대체, 상단 배너 참고). 이 화면 그룹은 그 전제를 사용자에게 처음 설명하고 동의를 받는 지점이다.
...
- .ai 근거 문서
  - `.ai/product/policies.md` — §1 사용자 고지(권한 문구 확정, 2026-07-26), §3 카메라 필수
  - `.ai/product/voice-tone.md` — §4 "카메라 권한 (S2-2 · S2-3)" 문구 표 (그대로 인용, 의역 금지)
  - `.ai/product/design.md` — 플랫폼 분기 원칙(2026-07-26), 화면 인벤토리 (V1.0 최종)
  - `.ai/product/user-flow.md` — 핵심 플로우 / 예외 플로우 "카메라 권한 거부"
  - `.ai/product/mvp-scope.md` — 세션 UX 정책 "최초 시작 (2026-07-26 확정)"
  - `.ai/notes/2026-07-26-디자인-반영-인터뷰-6차.md` — 카메라 권한 문구 확정, 최초 시작 플로우
  - `.ai/project/glossary.md` — 앱명 `FocusON`, 카메라 동작 동사는 "측정"으로 통일
- Ownership: `frontend/docs/screen-ownership.md` — S2-2 · S2-3 모두 `apps/mobile` 소유
- 담당 앱: `apps/mobile`

Figma가 시각적 SSOT, `.ai`가 문구·정책의 SSOT다. 구현 전 `get_design_context`로 노드 `52:312`(S2-3)를 반드시 재확인한다. 절대 좌표를 그대로 베끼지 말고 Flexbox + SafeArea 레이아웃으로 매핑한다.
...
모든 문구는 `.ai/product/voice-tone.md` §4 "카메라 권한 (S2-2 · S2-3)"에서 **그대로** 가져왔고, Figma 텍스트 노드와 문자 단위로 일치함을 확인했다. 의역·줄임·문장부호 변경 금지.
...
프라이버시 캡션은 **싱글룸 문구**다. V1.0 화면 인벤토리에는 멀티룸 화면이 없으므로(`design.md` — S7~~S11은 V1.2~~V1.4) 멀티룸 표현("AI 분석용 원본 프레임·얼굴 데이터가 서버로 전송되지 않아요")을 이 화면에 절대 가져오지 않는다. 과거 fe 문서(2026-07-27 삭제된 구 ADR 0002 등)가 멀티룸을 서술했더라도 마찬가지다.
...
`.ai` 어디에도 확정된 서술이 없다. 선택지는 ① 복귀 즉시 S2-3을 닫고 세션을 자동 시작 ② S2-3을 닫고 홈으로만 복귀(세션은 사용자가 다시 "집중 시작"을 눌러야 시작) ③ S2-3에 머무르며 아무것도 하지 않음. 빌더는 **임의로 확정하지 말고** 다음처럼 처리한다:
...
- **Figma의 S2-2 배경과 확정 플로우의 괴리** — Figma `52:139`는 다이얼로그를 **S1 홈 위에** 얹어 그렸지만, `mvp-scope.md`·`user-flow.md`의 확정 플로우에서 권한 요청 시점은 **G5(온보딩 마지막) 이후**다. 다이얼로그 뒤 배경은 OS가 통제하지 않는 앱 화면이므로 구현에는 영향이 없으나, **"홈에서 집중 시작을 누르는 즉시 권한을 요청한다"로 오독하면 안 된다.** .ai가 최신(2026-07-26)이므로 플로우는 .ai를 따르고, Figma 시안 갱신은 Review Checklist로 남긴다.

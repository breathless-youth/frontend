...
11. **싱글룸 프라이버시 문구만 쓴다**: `영상은 기기 안에서만 처리돼요`. 과거 fe 문서(2026-07-27 삭제된 구 ADR 0002 등)가 LiveKit 멀티룸을 서술했지만 **V1.0 화면 인벤토리에 멀티룸은 없다** — "AI 분석용 원본 프레임이 서버로 전송되지 않는다" 계열 문구를 이 화면에 끌어오지 않는다.

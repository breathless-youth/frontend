...
- [ ] **세션 중(다이얼로그 없이) Android 하드웨어 뒤로가기 동작 확정 — 현재 "차단"으로 구현됨**(2026-07-30). 이 문서는 다이얼로그가 열린 상태의 뒤로가기만 다뤘고, 세션 중 뒤로가기는 어느 스펙에도 없었다. 막지 않으면 네이티브가 세션 화면을 팝하면서 WebView가 파괴되고 `endAndSubmit`이 실행되지 않아 **세션이 통째로 유실된다**(경고·복구 없음). 위 표의 "종료를 이 경로로 확정시키지 말 것" 원칙과 U1 `UpdateNoticeSheet`의 미확정 닫기 경로 처리(막아 둔다)를 따라 **차단**을 잠정 기본값으로 구현했다(`apps/mobile/app/room/[id].tsx`). 세션이 살아 있을 때만 막고 오류·로딩 화면에서는 막지 않는다(갇힘 방지).

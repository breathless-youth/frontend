...
- `colors.border.default` — 요약 카드 테두리
...
### 토큰 스케일 밖 실측값 (근사시키지 말고 실측값 그대로 사용)

- 버튼 반경 **14px** — `radius` 스케일에 없다(12 또는 16으로 반올림하지 말 것).
- 다이얼로그 그림자 `0 20px 50px rgba(0,0,0,0.45)` — 그림자 토큰이 아직 없다.
- 요약 카드 구분선 `#EFF1F3` — 시맨틱 토큰에 없는 값. `colors.border.default.light`(#E5E8EB)와 다르다.

...
10. V1.0 범위 밖 요소를 추가하지 않는다(로그인, 소셜/멀티룸, 목표 시간, 뽀모도로, 알림·진동). 특히 **자동 종료 시 푸시/사운드 알림을 붙이지 말 것** — 알림 정책은 미정이고, **세션 중 알림음·진동 미사용이 확정 정책이다.**
    - **2026-09-20 개정**: 원래 이 항목은 "알림·소리·진동"을 한 묶음으로 금지했다. 그 취지는 **앱이 사용자 의사와 무관하게 내는 소리**를 막는 것이었는데, 문구가 넓어 사용자가 직접 켠 배경음까지 막고 있었다. 금지 대상을 **알림음과 진동**으로 좁힌다.
    - **사용자가 세션 화면에서 직접 켠 배경음은 허용한다**(백색소음·자연음 재생, BY-682). 기본값은 꺼짐이고, 사용자가 켜지 않으면 어떤 소리도 나지 않는다. 자동 종료·상태 전환 같은 시스템 사건에 소리를 붙이는 것은 여전히 금지다.

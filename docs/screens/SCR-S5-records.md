...
달력에서 날짜를 선택하면 그 아래 요약과 리스트가 해당 날짜 기준으로 갱신되는 구조다. V1.0에서는 통계 고도화(히트맵·주간/월간 추이·정렬 토글)를 하지 않는다 — 그것들은 전부 M2+ 범위다(`ai-wiki/product/user-flow.md` S5 행, `ai-wiki/product/design.md` "S5 기록 구조").
...
- ai-wiki 근거 문서:
  - `ai-wiki/product/design.md` — "V1.0 최종 확정"의 **S5 기록 구조** 행, "확정 사항(2026-07-24)"의 **기록 달력** 행, "인터뷰 6차 확정"의 **달력 상세** 행
  - `ai-wiki/product/voice-tone.md` — §2 표기 규칙(시간 길이·시각 범위·날짜·집중률 2형식·연속 공부), §4 **기록 (S5)** 표준 문구
  - `ai-wiki/project/glossary.md` — 순공시간·총 공부 시간·집중률·연속 공부·공부 횟수·공부(세션 단위)·비집중·일시정지 노출 표기
...
### ⚠️ 이벤트 카운트가 실제 횟수보다 작을 수 있다 (상류 미해결 — 리더 결정 대기)
...
- **기록이 아예 없을 때(설치 직후 첫 사용)**: **빈 상태 미정 — 확인 필요.** Figma에도 `ai-wiki`에도 정의가 없다(Figma 컴포넌트 검색 결과 empty-state 컴포넌트 없음). 특히 **연속 공부 0일일 때의 배너 문구가 없다** — S1 홈의 0일 문구(`오늘 10분 집중하면 연속 공부가 시작돼요`)는 홈 스탯 카드용으로 확정된 것이라 이 배너에 그대로 복사하지 않는다.
...
5. 이 프레임에는 iOS 상태바가 없다. `SafeAreaView`를 쓰고, 헤더 `기록` 타이틀의 상단 여백은 같은 탭 화면인 S6(설정, `67:722` — 상태바 아래 17px)와 **동일한 기준으로 맞춘다**. Figma의 y=40은 상태바가 빠진 스크롤 캔버스 좌표라 그대로 쓰면 S6와 어긋난다(→ Review Checklist).
...
- 달력 셀의 시각 크기는 46×44이지만 **터치 타겟은 최소 44×44를 보장**한다(현재 높이 44 충족, 폭 46 충족). 월 이동 버튼(32×32)은 **시각 크기는 유지하고 히트슬롭으로 44×44를 확보**한다.
...
- **Figma ↔ wiki 괴리(확인 필요, 임의 선택하지 않음)**: 정렬 컨트롤이 Figma에는 `최신순` + `chevron-down`(드롭다운처럼 보임)으로 그려져 있으나, `ai-wiki`(design.md 6차 확정 · voice-tone §4)는 **V1.0 최신순 고정, 토글은 M2+**로 확정했다. → **wiki를 따라 비인터랙티브로 구현**하고(2026-07-26 기준 wiki가 최신), 셰브런을 시각적으로 남길지 제거할지는 디자이너 확인 대상으로 남긴다.
...
- [ ] **일시정지 칩의 배경 토큰** 확정(회색 subtle 토큰 신설 여부) — 현재 `state/distractSubtle`의 회색 대응물이 없음
- [ ] **`eventCounts` 하향 편차 정책 결정**(ⓐ버림 유지 / ⓑ1초 올림 / ⓒ입력단 디바운스) — `toStatusEvents`와 `computeSessionTotals`의 1초 미만 구간 처리 불일치. **결정 범위는 `PAUSE` 단독이 아니라 `StudyEventStatus` 4종 전체다**(재현 경로 3종 확인 — Data Contract 해당 절 참조). ⓑ를 택하면서 PAUSE만 고치면 `AWAY`·`PHONE`·`DEVICE` 편차가 남는다. 결정 후 Data Contract의 해당 절과 칩 규칙을 함께 갱신. 제보·실행검증 `spec-WG2` / 실측 `qa-WG1`
...
- [ ] S5 프레임에 iOS 상태바가 없어 헤더 상단 여백 기준이 S6와 달라 보인다 — S6(상태바 아래 17px) 기준으로 통일해도 되는지 확인

...
> **BY-257(2026-07-27) 이후 구성이 Figma와 다르다.** 아래 "Current Figma Structure"·"Content"는 **디자인 파일의 기록**이고, 실제 구현은 다음 두 가지가 다르다:
>
> - **`측정 기준 안내` 행 제거** — 온보딩 가이드(G1~G5)는 그대로 살아 있고 홈의 가이드 카드로 재진입한다. 설정에서 가는 길만 없앴다.
> - **`오픈소스 라이선스` 행 제거** — 목적지도 문서도 없던 행이다.
>
...
- **온보딩 가이드(G1~G5)와 이제 아무 관계가 없다.** BY-257에서 "측정 기준 안내" 행을 제거해 설정발 진입 경로가 사라졌다. 가이드용 설명 문구를 이 화면에 다시 쓰지 말 것 — 가이드는 MG5(`SCR-G1-G5-onboarding-guide.md`)가 소유하고 홈의 가이드 카드로 진입한다.
...
| ~~행 2 라벨~~    | ~~`측정 기준 안내`~~ — BY-257에서 제거                                               | Figma `43:97`                                                                      |
| ~~행 2 서브~~    | ~~`자리 이탈 · 휴대폰 사용 · 기기 조작을 기기 안에서만 측정해요`~~ — BY-257에서 제거 | `voice-tone.md` §4 설정(S6) "측정 기준 안내 서브" — Figma `43:98`과 문자 단위 일치 |
...
| 행 4~5 라벨      | `이용약관` / `개인정보처리방침` (`오픈소스 라이선스`는 BY-257에서 제거)              | Figma `67:828` / `67:832` / `67:836`                                               |
...
3. **정적 진입점 4곳의 목적지** — 2026-07-27(BY-257)에 3곳이 확정됐다. **API 계약이 아니다.**
   ...
   - **오픈소스 라이선스 — 행 자체를 제거했다**(BY-257). 목적지도 문서도 없었다.
...
| ~~4~~   | ~~행 `측정 기준 안내`~~                     | **BY-257에서 행을 제거했다.** 온보딩 가이드는 홈의 가이드 카드로만 재진입한다 — `design.md`의 "설정 '측정 기준 안내'에서 재진입"과 `voice-tone.md` G5 힌트("이 안내는 설정 > 측정 기준 안내에서 언제든 다시 볼 수 있어요")가 이제 사실과 다르다. | ⚠️ 제거됨 — wiki 문구 갱신 필요                                                                                     |
| ~~4-a~~ | ~~재진입 시 G5의 CTA `집중 시작하기` 동작~~ | 설정발 재진입 경로가 사라져 이 화면에서는 해당 없음. `lib/onboardingGuideSteps.ts`의 진입 출처 `"settings"`는 MG5 소유라 그대로 뒀다(현재 참조하는 곳 없음).                                                                                     | — 이 화면 범위 밖                                                                                                   |
...
| ~~6-a~~ | ~~행 `오픈소스 라이선스`~~                  | **BY-257에서 행을 제거했다.** 목적지도 문서도 없었다.                                                                                                                                                                                            | ⚠️ 제거됨                                                                                                           |
...
- **미확정 목적지가 남아 있지 않다** — 문의 폼·이용약관·개인정보처리방침 3곳이 BY-257에서 확정됐고, 목적지가 없던 오픈소스 라이선스 행은 제거했다(Data Contract 3 참조).
- **`user-flow.md`·`voice-tone.md`·`design.md`가 아직 제거된 두 행을 전제한다.** `user-flow.md` S6 정의는 "측정 기준 안내=가이드 재진입", "문의하기 외부 폼", "오픈소스 라이선스"를 그대로 두고 있고, `voice-tone.md` G5 마지막 힌트는 "설정 > 측정 기준 안내에서 언제든 다시 볼 수 있어요"라고 말한다. **wiki 쪽 문구를 갱신하지 않으면 다음 구현자가 되돌린다.**
...
- [x] ~~**오픈소스 라이선스** 목적지 확정~~ — 행을 제거해 해소 (BY-257)
- [ ] **wiki 문구 갱신** — `user-flow.md` S6 정의(측정 기준 안내·문의하기 외부 폼·오픈소스 라이선스), `voice-tone.md` G5 마지막 힌트("설정 > 측정 기준 안내에서 언제든 다시 볼 수 있어요"), `design.md`의 "설정 '측정 기준 안내'에서 재진입"이 전부 현재 구현과 어긋난다.
- [ ] **Figma 원본을 현재 구성에 맞추기** — `측정 기준 안내`(`67:749`)·`오픈소스 라이선스`(`67:836`) 행 삭제, 문의하기(`67:759`) 트레일링을 external-link → chevron. 고치지 않으면 다음 익스포트에서 되돌아간다.

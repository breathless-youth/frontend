...
6. **새 npm 의존성을 추가로 설치하지 않는다.** 필요한 것은 이미 다 있다 — `expo-constants`(버전), `react-native`의 `Linking`(설정 열기·외부 링크), `react-native-svg`(아이콘), `expo-camera`(권한 조회 — 2026-07-27 도입, [ADR 0004](../adr/0004-expo-camera-for-permission-api-only.md)). 카메라 SDK는 `lib/cameraPermission.ts` 어댑터를 통해서만 쓰고 **화면에서 직접 import하지 않는다**.
...
- [ ] **S6에 "측정 방식(자동/수동)" 행 추가** — `user-flow.md` S6·`policies.md` §3(2026-07-26) 확정 항목이나 아직 없다. 수동 타이머 모드와 함께 별도 티켓. 이 행이 들어오면 위 "이 화면에서 앱이 직접 바꾸는 상태는 하나도 없다"는 전제가 바뀐다.
- [ ] **문의 폼 URL** 확정 (외부 폼 주소)

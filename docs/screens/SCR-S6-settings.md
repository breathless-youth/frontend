...
- **로그인·계정 관련 항목이 없다 — 미포함(V1.2+)이다.** `policies.md` §2: V1.0~V1.1은 익명 기기 계정, 로그인 도입은 V1.2(Google/Apple)부터. `user-flow.md` S6도 "로그아웃·계정 삭제 없음"으로 명시. `design.md` 화면 인벤토리에서 S0 로그인은 V1.2 항목이다. (V1.2 로그인 예고를 담당하던 U1 업데이트 안내 시트는 2026-08-16 삭제됐다 — 예고가 필요해지면 공지 팝업(U1, BY-377)으로 안내한다.)

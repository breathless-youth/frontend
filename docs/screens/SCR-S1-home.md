...
- **두들 일러스트의 다크모드 색은 코드에서 토큰으로 복원한 것이다**(2026-07-26). Figma 원본은 이 일러스트(노드 `32:94`)에 변수 바인딩 없이 색을 하드코딩해뒀는데, 부모 가이드 카드(`39:108`)의 배경은 `bg/guide` 토큰에 바인딩돼 있어 다크모드에서 어두워진다 — 그 결과 윤곽선(`#191F28`)이 배경에 묻혀 그림이 판독 불가능해졌다(Figma에서도 동일하게 깨지는 상태). 사용된 색이 전부 시맨틱 토큰의 라이트값과 정확히 일치해서 원래 의도됐을 바인딩을 복원하는 방식으로 고쳤다(`text/primary`·`bg/base`·`bg/layer-2`·`brand/primary`·`text/tertiary`). 시뮬레이터에서 라이트·다크 모두 정상 확인. 디자이너가 다크 전용 두들 변형을 별도로 원한다면 그때 교체한다.
...
- [ ] **Figma의 두들 일러스트(`32:94`)에 색상 변수 바인딩 추가** — 현재 하드코딩이라 Figma 자체 다크모드에서 그림이 배경에 묻힌다. 코드는 토큰 바인딩으로 이미 고쳤으나, Figma 원본도 맞춰두지 않으면 다음 익스포트 때 같은 문제가 반복된다. 다크 전용 두들 변형을 따로 만들 계획이라면 그쪽으로 확정.

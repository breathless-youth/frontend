...
| 타임라인 범례         | 2색(집중·비집중) — `64:572`                  | **3색: 집중(블루) · 자동 멈춤(오렌지) · 일시정지(회색, 있을 때만)** (`design.md` 6차 "S4 타임라인 범례"; `비집중`→`자동 멈춤` 표기는 2026-09-14 BY-560)                            |
...
| 타임라인 범례        | `집중` · `자동 멈춤` · `일시정지`(있을 때만) — 카드 헤더 오른쪽                                            | design.md 6차 S4 타임라인 범례 → 2026-09-14 BY-560 시안으로 `비집중`을 `자동 멈춤`으로 교체(이 화면 한정, 전면 교체는 BY-574)                                                                                                                                                                                           |
...
| 요약 카드 행 2       | `누적 공부 일 수` · `{N}일`                                                                                | BY-560 시안. 지금까지 공부 기록이 있는 날(KST `statDate`) 수 — `GET /api/stats/period?from=2026-01-01&to=오늘`의 `dailyList`에서 `studySec > 0`인 날을 센다(366일 상한이라 창을 나눠 합침, `useResultSummary.countStudyDays`). 스트릭 기준(10분)이 아니라 기록 화면과 같은 기준(순공 1분 미만 제외). 조회 중·실패는 `—` |

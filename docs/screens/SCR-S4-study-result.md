...
| 통계 행 값           | 접힘 `{N}회` / 펼침 `총 {시간 길이}` + 발생 구간 목록  | design.md 6차 / voice-tone §4 — 접힘·펼침 분리는 2026-07-27, 구간 목록은 2026-07-31(BY-336). 아래 절 참고                           |
| 비집중 0             | `비집중 없이 이어간 공부예요` (느낌표 없음)            | voice-tone §4                                                                                                                       |
| CTA                  | `확인`                                                 | voice-tone §4                                                                                                                       |

### 통계 행 펼침 — 합계 + 발생 구간 (2026-07-31, BY-336)

행은 접힌 상태에서 **횟수만** 보이고, 누르면 그 자리에서 아래가 펼쳐진다(Figma 시안 없음 — 펼침 자체가 Figma 이후 결정이다).
...
- **왜 합계만으로 부족한가**: `3회 9분`이 5분 한 번 + 2분 두 번인지 3분씩 고르게인지 구분되지 않는데, 사용자가 알고 싶은 것은 대개 "언제 무너졌나"다.
- 시각은 **세션 벽시계**라 위 타임라인 바의 세그먼트 위치와 그대로 대응된다 — 바에서 본 구간을 목록에서 확인하는 흐름이 성립한다.
- 정렬·병합은 하지 않는다. 서버가 시작 시각 오름차순을 보장한다(아래 Data Contract).
- 구간 길이도 **분 단위 표기**를 따른다(초 금지) — 1분 미만 구간은 `1분 미만`으로 나온다.
- ⚠️ 개별 구간은 자기 길이만 내림하고 합계는 ms 누적에서 한 번만 내림한다. 그래서 **구간들의 합이 합계보다 최대 (건수−1)초 작을 수 있다.** 분 단위 표기라 화면에 드러나지 않으며, 합계 쪽이 정확한 값이다.

...
| --------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 통계 카드 4번째 행    | `화면 꺼짐` + 오렌지 도트(`64:642`~`64:646`) | **행 라벨을 `일시정지`로, 도트를 회색(`text/tertiary` #8B95A1)으로.** 비집중 3종 행과 시각적으로 구분된다. (2026-09-14 BY-560: 통계 카드 자체가 요약 카드로 대체돼 더는 해당 없음) |
| 통계 카드 타이틀 합계 | `비집중 21분` (화면 꺼짐 3분 포함)           | **`비집중 18분`** — 일시정지는 비집중 합계에서 제외한다. (2026-09-14 BY-560: 카드 대체로 해당 없음)                                                                                |
| 일시정지 행 노출 조건 | 항상 노출                                    | **일시정지가 1회 이상일 때만 노출** (`user-flow.md` S4 행: "일시정지 행(있을 때만)"). (2026-09-14 BY-560: 카드 대체로 해당 없음)                                                   |
| 타임라인 범례         | 2색(집중·비집중) — `64:572`                  | **3색: 순공(블루) · 자동 멈춤(오렌지) · 일시정지(회색, 있을 때만)** (`design.md` 6차 "S4 타임라인 범례"; `집중`→`순공`은 glossary, `비집중`→`자동 멈춤`은 이 화면 한정 시안 표기)  |
| 타임라인 바 세그먼트  | 오렌지 세그먼트만 (`64:564`~`64:568`)        | **오렌지(자동 멈춤) + 회색(일시정지) 세그먼트 모두 렌더**                                                                                                                          |
...
| 위치                 | 문구                                                                                                       | 출처                                                                                                                                                        |
| -------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 히어로 타이틀·설명   | `오늘 공부 완료!` · `끝까지 해낸 시간이 그대로 기록됐어요`                                                 | BY-557 시안 프로토타입(BY-560, 2026-09-13) — voice-tone 미등재, 시안 문구 그대로. 옛 타이틀 `공부 결과`(voice-tone §4)는 시안에서 빠짐                      |
| 히어로 라벨          | `순공시간`                                                                                                 | glossary 노출 표기(시안의 `순공 시간` 띄어쓰기는 BY-574 범위)                                                                                               |
| 히어로 값            | `{N}시간 {M}분` (시간 길이 규칙) — 0에서 카운트업                                                          | voice-tone §2 · 카운트업은 BY-557 시안                                                                                                                      |
| 집중률 배지          | `{N}% 집중` (링 아이콘 + 텍스트)                                                                           | voice-tone §2 집중률 — **필/헤더는 "N% 집중" 형식**("집중률 N%"는 지표 라벨 형식이라 여기 쓰지 않는다)                                                      |
| 히어로 메타          | `총 공부시간 {N}시간 {M}분` — 시각 범위는 없음                                                             | design.md 6차 S4 헤더 · BY-557 시안. `{HH:MM} – {HH:MM}`은 타임라인 축 라벨이 갖는다(BY-560에서 히어로에서 제거)                                            |
...
| 타임라인 범례        | `순공` · `자동 멈춤` · `일시정지`(있을 때만) — 카드 헤더 오른쪽                                            | design.md 6차 S4 타임라인 범례 → `집중`은 glossary의 `순공`, `비집중`은 glossary `휴식` 대신 시안의 `자동 멈춤`(이 화면 한정 예외)                          |
| 최고 집중 시간       | 바 위 배지 `최고 집중 시간 {N}분` + 바 안 간격 분리 조각 + 행 `최고 집중 시간 · {N}분 · {HH:MM} – {HH:MM}` | BY-560 시안 스크린샷(2026-09-14). 이벤트로 끊기지 않은 가장 긴 구간(`sessionResult.longestFocusStretch`), 없으면 셋 다 숨김                                 |
| 요약 카드 행 1       | `오늘 누적 순공시간` · `{시간 길이}`(강조색)                                                               | BY-560 시안 스크린샷(2026-09-14) — 옛 비집중 통계 카드(`비집중 {N}분` + 유형별 행 + 펼침, BY-336)를 대체. 값은 `GET /api/stats?date=오늘`의 `totalFocusSec` |
| 요약 카드 행 2       | `누적 공부 일 수` · `{N}일`                                                                                | 시안. 값은 `GET /api/stats/study-days?userId=`의 `totalDays`(지금까지 공부 기록이 있는 날 수). 조회 중·실패는 `—`                                           |
| CTA                  | `홈으로` · `기록으로 가기`                                                                                 | BY-557 시안 프로토타입(BY-560). 옛 단일 `확인`(voice-tone §4)은 제거                                                                                        |
...
| 요소                         | 동작                                                                                                                                                                                                                                |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| CTA `홈으로` (BY-560)        | **홈(S1)으로 돌아간다** (`user-flow.md`: `F → A`; 소셜 결과는 소셜 홈). S4는 WebView 안에서 돌기 때문에 웹 라우터만으로는 모바일 홈 탭에 갈 수 없다 → 아래 "호스트 복귀" 참고                                                       |
| CTA `기록으로 가기` (BY-560) | **기록 탭(S5)으로 간다.** 웹뷰에서는 솔로가 모달 닫기와 탭 전환을 한 메시지 `navigate-home {tab: records}`로, 소셜(탭 웹뷰)이 `navigate-tab {via: study_result}`로 보낸다. 브라우저 단독 모드에서는 웹 라우트 `/records`(쿼리 승계) |

두 CTA는 도장 연출이 끝나 카드가 공개된 뒤(`RESULT_REVEAL_DELAY_MS` 3.5s)에만 있다. 예전 단일 CTA `확인`(`64:647`)과 우상단 `X`(`64:553`)는 BY-557 시안에서 빠져 2026-09-13 BY-560으로 제거됐다.
...
Figma 실측값 중 `typography` 스케일에 없는 것이 많다. S1 홈의 선례대로 **억지로 스케일에 맞추지 말고 실측값을 그대로 쓴다**.
...
> 2026-09-14(BY-560) 갱신: 사용자 요청으로 이 화면에서 소비자가 둘 이상인 조각만 shadcn 스타일 프리미티브로 승격했다 — `components/ui/card.tsx`(Card·CardHeader·CardTitle(h2)·CardContent), `badge.tsx`(elevated=집중률 · outline=최고 집중), `separator.tsx`, `button.tsx`에 `secondary` 변형·`xl` 사이즈 추가. 라딕스 의존은 더하지 않았다. 상태 도트·바 세그먼트(`ResultCardParts.tsx`)는 이 화면 전용이라 feature에 남겼다.

| 컴포넌트                                                              | 역할                                                                                                                                  |
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| `ResultPage`                                                          | 라우트 진입점 (`apps/web/src/routes/ResultPage.tsx`)                                                                                  |
| `StudyCompleteHero`                                                   | (BY-560) 도장 모션 · `오늘 공부 완료!` · 순공시간 카운트업 · 총 공부 · 집중률 배지 — 옛 `ResultHeader` 대체                           |
| `ConfettiBurst`                                                       | (BY-560) 도장이 닿는 순간 터지는 색종이 캔버스(장식, 모션 축소에서 꺼짐)                                                              |
| `StudyTimelineCard`                                                   | 헤더 타이틀+3색 범례(조건부) · 최고 집중 배지·간격 분리 조각 · 타임라인 바 + 축 라벨 · 최고 집중 행(BY-560)                           |
| `SessionSummaryCard` + `useResultSummary`                             | (BY-560, 2026-09-14) `오늘 누적 순공시간`·`누적 공부 일 수` 두 행 — 옛 `DistractionStatsCard` 대체, S4가 서버 통계를 읽는 유일한 지점 |
...
아이콘은 `icon/close`(`64:554`) 하나뿐이다. Figma에서 SVG로 내보내 쓰고, **PNG를 쓰지 않는다** — Figma의 PNG 익스포트에는 캔버스 배경 `<rect>`가 합성돼 흰 네모로 보이는 문제가 S1 구현에서 실제로 발생했다. 단색 아이콘이므로 `currentColor`로 틴팅한다.
...
구현체: `apps/web/src/routes/ResultPage.tsx` + `features/study-session/{sessionResult.ts, resultCopy.ts, components/StudyCompleteHero.tsx, components/ConfettiBurst.tsx, components/StudyTimelineCard.tsx, components/SessionSummaryCard.tsx, useResultSummary.ts, components/ResultCardParts.tsx}`. 라우트는 `App.tsx`에 `/room/:id/result`로 등록했다.
...
| **비집중 0 + 일시정지 ≥1**     | 확정 문구 `비집중 없이 이어간 공부예요` 아래에 **일시정지 행만** 노출.                                                                                                                                          | (2026-09-14 카드 제거 — 해당 없음)     |

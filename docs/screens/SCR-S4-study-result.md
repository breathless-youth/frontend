...
| --------------------- | -------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| 통계 카드 4번째 행    | `화면 꺼짐` + 오렌지 도트(`64:642`~`64:646`) | **행 라벨을 `일시정지`로, 도트를 회색(`text/tertiary` #8B95A1)으로.** 비집중 4종(자리 이탈·휴대폰 사용·기기 조작·졸음) 행과 시각적으로 구분된다 |
...
| 통계 카드 타이틀     | `비집중 {N}분` (비집중 4종 합계만)                     | voice-tone §4                                                                                                                       |
...
| 통계 `졸음` 행 ⚠️             | `events` 중 `SLEEP`                         | Swagger 등재 전 잠정(BY-702). 이 PR은 감지를 연결하지 않아 실제로 발생하지 않는다          |
| 통계 `일시정지` 행            | `events` 중 `PAUSE`                         | 0건이면 행 자체를 렌더하지 않는다                                                          |
| 통계 카드 타이틀 `비집중 N분` | `AWAY`+`PHONE`+`DEVICE`+`SLEEP` 시간 합     | **`PAUSE` 제외**                                                                           |
...
| `DistractionStatsCard`                                                | 비집중 4종 행 + 일시정지 행(조건부) + 비집중 0 문구                                               |
